...
export async function markReportReviewing(report: RootReviewReport, moderatorUserId: string) {
  await updateDoc(doc(db, REVIEW_REPORTS_ROOT_COLLECTION, report.reportId), { status: "reviewing", updatedAt: serverTimestamp(), moderatedBy: moderatorUserId, moderatedAt: serverTimestamp() });
  await audit({ report, moderatorUserId, action: "mark_reviewing" });
}

export async function dismissReport(report: RootReviewReport, moderatorUserId: string, note?: string) {
  await updateDoc(doc(db, REVIEW_REPORTS_ROOT_COLLECTION, report.reportId), { status: "dismissed", moderationNote: note || "", updatedAt: serverTimestamp(), moderatedBy: moderatorUserId, moderatedAt: serverTimestamp() });
  await audit({ report, moderatorUserId, action: "dismiss_report", note });
}

export async function hideReviewForModeration(report: RootReviewReport, moderatorUserId: string, note?: string) {
...
export async function restoreReviewForModeration(report: RootReviewReport, moderatorUserId: string, note?: string) {
...
export async function addModerationNote(report: RootReviewReport, moderatorUserId: string, note: string) {
  await updateDoc(doc(db, REVIEW_REPORTS_ROOT_COLLECTION, report.reportId), { moderationNote: note.trim(), updatedAt: serverTimestamp(), moderatedBy: moderatorUserId, moderatedAt: serverTimestamp() });
