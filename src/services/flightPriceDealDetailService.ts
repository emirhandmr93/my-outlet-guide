...
export type RollingRouteUserFlightPriceDeal = {
  ...
  matchedThreshold: Threshold; metThresholds: Threshold[]; selectedThresholds: Threshold[]; trackingDayCount: number;
  historyWindowDays: 14 | 30 | 90; priceSampleCount: number; provider: "aviasales_data_api"; currency: "EUR";
  priceScope: "cached_offer"; passengerCountApplied: false;
};

export type UserFlightPriceDeal = ExactDateUserFlightPriceDeal | RollingRouteUserFlightPriceDeal;
...
function validateExactDateUserFlightPriceDeal(data: unknown, userId: string, eventId: string): ExactDateUserFlightPriceDeal | null {
  if (!isRecord(data) || Object.keys(data).some(key => !exactAllowedKeys.has(key)) || !isTimestamp(data.createdAt) || !isTimestamp(data.updatedAt)) return null;
...
const rollingAllowedKeys = new Set([
  "schemaVersion", "alertSchemaVersion", "eventId", "userId", "alertId", "queryKey", "providerQueryKey", "originAirportCode",
  "destinationAirportCode", "tripType", "tripClass", "directOnly", "monitoringMode", "monitoringWindowDays", "snapshotDate",
  "offerDepartDate", "offerReturnDate", "offerTransfers", "offerAirline", "offerFlightNumber", "offerSourceFoundAt", "currentPrice",
  "averagePrice", "discountPercent", "matchedThreshold", "metThresholds", "selectedThresholds", "trackingDayCount", "historyWindowDays",
  "priceSampleCount", "provider", "currency", "priceScope", "passengerCountApplied", "createdAt", "updatedAt",
]);
const safeOfferString = (value: unknown): value is string => typeof value === "string" && value.trim() === value && value.length > 0 &&
  new TextEncoder().encode(value).length <= 200 && !/[\u0000-\u001f\u007f-\u009f]/.test(value);
const addDays = (value: string, days: number) => new Date(Date.parse(`${value}T00:00:00Z`) + days * 86_400_000).toISOString().slice(0, 10);
const rollingKey = (data: Record<string, unknown>) => `${String(data.originAirportCode).toLowerCase()}_${String(data.destinationAirportCode).toLowerCase()}_rolling_route_365_${data.tripType}_${data.tripClass}_${data.directOnly ? "direct" : "any"}_eur`;

function validateRollingRouteUserFlightPriceDeal(data: unknown, userId: string, eventId: string): RollingRouteUserFlightPriceDeal | null {
  if (!isRecord(data) || Object.keys(data).some(key => !rollingAllowedKeys.has(key)) || !isTimestamp(data.createdAt) || !isTimestamp(data.updatedAt)) return null;
  const origin = data.originAirportCode; const destination = data.destinationAirportCode;
  if (data.schemaVersion !== 2 || data.alertSchemaVersion !== 3 || data.eventId !== eventId || data.userId !== userId || !EVENT_ID.test(eventId) ||
    typeof data.alertId !== "string" || data.alertId.length === 0 || data.queryKey !== data.alertId || data.providerQueryKey !== data.queryKey ||
    data.providerQueryKey !== rollingKey(data) || typeof origin !== "string" || !IATA.test(origin) || typeof destination !== "string" ||
    !IATA.test(destination) || origin === destination || (data.tripType !== "round_trip" && data.tripType !== "one_way") ||
    (data.tripClass !== "economy" && data.tripClass !== "business") || typeof data.directOnly !== "boolean" ||
    data.monitoringMode !== "rolling_route" || data.monitoringWindowDays !== 365 || !isDate(data.snapshotDate) || !isDate(data.offerDepartDate) ||
    data.offerDepartDate < data.snapshotDate || data.offerDepartDate > addDays(data.snapshotDate as string, 364)) return null;
  if ((data.tripType === "round_trip" && (!isDate(data.offerReturnDate) || data.offerReturnDate < data.offerDepartDate)) ||
    (data.tripType === "one_way" && Object.prototype.hasOwnProperty.call(data, "offerReturnDate"))) return null;
  if (!isInteger(data.offerTransfers, 0) || (data.directOnly && data.offerTransfers !== 0) ||
    ["offerAirline", "offerFlightNumber", "offerSourceFoundAt"].some(field => data[field] !== undefined && !safeOfferString(data[field]))) return null;
  if (typeof data.currentPrice !== "number" || !Number.isFinite(data.currentPrice) || data.currentPrice <= 0 ||
    typeof data.averagePrice !== "number" || !Number.isFinite(data.averagePrice) || data.averagePrice <= 0 ||
    typeof data.discountPercent !== "number" || !Number.isFinite(data.discountPercent) || data.discountPercent <= 0 ||
    !thresholds.has(data.matchedThreshold as Threshold) || !isThresholdArray(data.metThresholds) || !isThresholdArray(data.selectedThresholds)) return null;
  const met = data.metThresholds; const selected = data.selectedThresholds;
  if (met.at(-1) !== data.matchedThreshold || !selected.includes(data.matchedThreshold as Threshold) || met.some(value => !selected.includes(value)) ||
    !isInteger(data.trackingDayCount, 14) || (data.historyWindowDays !== 14 && data.historyWindowDays !== 30 && data.historyWindowDays !== 90) ||
    data.trackingDayCount < data.historyWindowDays || !isInteger(data.priceSampleCount, 1, data.historyWindowDays) ||
    data.provider !== "aviasales_data_api" || data.currency !== "EUR" || data.priceScope !== "cached_offer" || data.passengerCountApplied !== false) return null;
  const { createdAt: _createdAt, updatedAt: _updatedAt, ...deal } = data;
