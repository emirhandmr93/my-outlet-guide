...
import type { FlightDealAlertSaveInput, FlightDealThreshold, FlightDealTripClass, FlightDealTripType, RollingRouteFlightDealAlertSaveInput } from "./flightDealAlertService";
...
export type FlightDealAlertSubmissionResult =
  ...
  | { status: "sign_in_required" | "origin_required" | "destination_required" | "same_airport_error" | "trip_type_error" | "trip_class_error" | "direct_only_error" | "depart_date_required" | "return_date_required" | "past_date_error" | "return_before_departure" | "passenger_error" | "threshold_required" | "save_failed" };

type SaveRollingRouteFlightDealAlert = (userId: string, input: RollingRouteFlightDealAlertSaveInput, previousAlertId?: string) => Promise<string>;
...
export async function submitRollingRouteFlightDealAlert({ monitoringPubliclyVerified, userId, origin, destination, thresholds,
  ...
  active?: boolean;
  save: SaveRollingRouteFlightDealAlert;
}): Promise<FlightDealAlertSubmissionResult> {
  if (!userId) return { status: "sign_in_required" };
  if (!origin) return { status: "origin_required" };
  if (!destination) return { status: "destination_required" };
  if (origin.airportCode.trim().toUpperCase() === destination.airportCode.trim().toUpperCase()) return { status: "same_airport_error" };
  if (tripType !== "round_trip" && tripType !== "one_way") return { status: "trip_type_error" };
  if (tripClass !== "economy" && tripClass !== "business") return { status: "trip_class_error" };
  if (typeof directOnly !== "boolean") return { status: "direct_only_error" };
  if (!Array.isArray(thresholds) || thresholds.length === 0
    || thresholds.some(value => value !== 15 && value !== 30 && value !== 45)
    || new Set(thresholds).size !== thresholds.length) return { status: "threshold_required" };
  try {
    await save(userId, {
      originLabel: `${origin.cityName} (${origin.airportCode})`, originAirportCode: origin.airportCode,
      originAirportName: origin.airportName, originCityName: origin.cityName,
      originCountryCode: origin.countryCode, originCountryName: origin.countryName,
      destinationType: "airport", destinationKey: destination.airportCode,
      destinationAirportCode: destination.airportCode, destinationAirportName: destination.airportName,
      destinationCityName: destination.cityName, destinationCountryCode: destination.countryCode,
