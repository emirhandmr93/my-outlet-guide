...
import {
  hasSegmentDateOverlap,
  sortTripSegments,
} from "../services/tripReminderPlan";
import {
  expandSearchValues,
  normalizeSearchText,
...
function routeSearchRank(
  ...
  const cityValues = values.cityId
    ? ([
        values.cityName,
        values.cityId,
        formatCityDisplayName(values.cityId, language),
        ...expandSearchValues(values.cityName || values.cityId),
      ].filter(Boolean) as string[])
    : [];
  const outletValues = values.outletName ? [values.outletName] : [];
  const countryValues = values.countryId
    ? ([
        values.countryName,
        values.countryId,
        formatCountryDisplayName(values.countryId, language),
        ...expandSearchValues(values.countryName || values.countryId),
      ].filter(Boolean) as string[])
    : [];
  const matches = (items: string[], test: (value: string) => boolean) =>
    items.some((item) => test(normalizeSearchText(item)));
  if (matches(cityValues, (value) => value === normalizedQuery)) return 6000;
  if (matches(cityValues, (value) => value.startsWith(normalizedQuery)))
    return 5000;
  if (matches(outletValues, (value) => value === normalizedQuery)) return 4000;
  if (matches(outletValues, (value) => value.startsWith(normalizedQuery)))
    return 3500;
  if (
    matches(
      countryValues,
    ...
    return 2500;
  if (
    matches([...cityValues, ...outletValues, ...countryValues], (value) =>
...
export function TripSegmentEditorScreen() {
  ...
  const trip = trips.find(
    (item) =>
      item.id === route.params.tripId || item.tripId === route.params.tripId,
  );
  const existing = trip?.segments.find(
    (item) => item.id === route.params.segmentId,
  );
  const [title] = useState(existing?.title || "");
  const [routeQuery, setRouteQuery] = useState("");
  const [isEditingRoute, setIsEditingRoute] = useState(!existing);
  const [segment, setSegment] = useState<TripSegment>(
    existing || {
      id: `segment-${Date.now()}`,
  ...
  const routeResults = useMemo<RouteSearchResult[]>(() => {
    ...
    const outletResults = searchOutlets(routeQuery)
      .map((outlet: any) => ({
        score: routeSearchRank(
          {
            ...
            countryName: outlet.countryName,
        ...
        result: {
          ...
          subtitle: formatOutletLocationSubtitle(
            outlet.cityId,
            outlet.countryId,
            language,
          ),
    ...
    return [...cityResults, ...outletResults]
      .sort(
        (a, b) =>
  ...
  function selectCity(city: { cityId: string; countryId: string }) {
    setSegment((value) => ({
      ...value,
      cityId: city.cityId,
      cityName: formatCityDisplayName(city.cityId, language),
      countryCode: city.countryId,
      countryName: formatCountryDisplayName(city.countryId, language),
      outletId: undefined,
  ...
  function selectOutlet(outlet: any) {
    setSegment((value) => ({
      ...
      countryCode: outlet.countryId,
      countryName: formatCountryDisplayName(outlet.countryId, language),
    }));
    setRouteQuery("");
    setIsEditingRoute(false);
  }
  ...
  async function save() {
    ...
    if (!next.cityId && !next.outletId && !next.cityName && !next.outletName)
      return Alert.alert(
        t("createTrip.validationTitle"),
        t("tripSegment.cityOrOutletRequired"),
      );
    if (next.startDate < trip.startDate || next.endDate > trip.endDate)
      return Alert.alert(
        t("createTrip.validationTitle"),
        t("tripSegment.datesOutsideTrip"),
      );
    ...
    try {
      await updateTrip(trip.id, {
        segments: sortTripSegments([...others, next]),
      });
      navigation.goBack();
    } finally {
      setSaving(false);
  ...
  const selectedTitle =
    segment.outletName ||
    segment.cityName ||
    (segment.cityId ? formatCityDisplayName(segment.cityId, language) : "");
  const selectedSubtitle =
    segment.cityId && segment.countryCode
      ? formatOutletLocationSubtitle(
          segment.cityId,
          segment.countryCode,
          language,
    ...
    >
      <View style={styles.card}>
        <Text style={styles.sectionTitle}>
          {existing
            ? t("tripDetail.editRouteCta")
            : t("tripSegment.whereGoingTitle")}
        </Text>
          ...
          <>
            ...
            {routeQuery.trim().length === 0 && (
              <Text style={styles.emptyHelper}>
                {t("tripSegment.emptySearchHelper")}
              </Text>
            )}
              ...
              >
                <View style={styles.resultHeader}>
                  <Text style={styles.typePill}>
                    {t(
                      result.type === "city"
                        ? "tripSegment.cityType"
                        : "tripSegment.outletType",
                    )}
                  </Text>
                </View>
        ...
        {routeSelected && !isEditingRoute && (
          <View style={styles.summaryCard}>
            <View style={styles.summaryCopy}>
              ...
              {!!selectedSubtitle && (
                <Text style={styles.resultSubtitle}>{selectedSubtitle}</Text>
              )}
            </View>
            <TouchableOpacity
              style={styles.changeButton}
              onPress={editRouteSelection}
            >
              <Text style={styles.changeButtonText}>
                {t("tripSegment.changeRoute")}
              </Text>
            </TouchableOpacity>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  resultHeader: { flexDirection: "row", marginBottom: 6 },
  typePill: {
    alignSelf: "flex-start",
    backgroundColor: "#F2E8BF",
    borderRadius: 999,
    color: "#7A5D00",
    fontSize: 11,
    fontWeight: "900",
  ...
  resultTitle: { color: "#0B1F3A", fontWeight: "900" },
  resultSubtitle: { color: "#666", marginTop: 4 },
  summaryCard: {
    backgroundColor: "#F7F8FA",
    borderRadius: 18,
    padding: 14,
  ...
  changeButtonText: { color: "#FFFFFF", fontWeight: "900" },
