...
import { TripReminderPlanItem, TripSegment, useTrips } from "../contexts/TripsContext";
...
function ReminderRow({ item, t }: { item: TripReminderPlanItem; t: (key: string) => string }) {
  return <View style={styles.reminderRow}><Text style={styles.detailLabel}>{item.date} · {t(item.title)}</Text><Text style={styles.detailValue}>{applyParams(t(item.body), item.messageParams)}</Text></View>;
}
