...
import { Alert, Modal, Platform, ScrollView, StyleSheet, Text, TouchableOpacity, useWindowDimensions, View } from "react-native";
...
function ReminderRow({ item, t }: { item: TripReminderPlanItem; t: (key: string) => string }) {
  ...
  return <View style={styles.reminderRow}>
    <Text style={styles.detailLabel}>{formatIsoDateOnly(item.date)} · {t(item.title)}</Text>
...
function FlightLegCard({ label, leg, reminder, t }: { label: string; leg?: TripFlightLegDetails; reminder?: TripReminderPlanItem; t: (key: string) => string }) {
  ...
  const dateTime = [formatIsoDateOnly(leg?.departureDate), leg?.departureTime].filter(Boolean).join(" · ");
...
function parseDate(value: string) { return parseIsoDateOnly(value) ?? todayAtMidnight(); }
function formatDate(value: Date) { return localDateToIso(value); }

function SegmentRow({ segment, onEdit, onDelete, readOnly }: { segment: TripSegment; onEdit: () => void; onDelete: () => void; readOnly?: boolean }) {
  const place = [segment.cityName, segment.outletName].filter(Boolean).join(" · ") || segment.countryName || segment.title;
  return <View style={styles.segmentCard}><Text style={styles.detailValue}>{formatIsoDateOnly(segment.startDate)} – {formatIsoDateOnly(segment.endDate)} · {place}</Text>{segment.countryName ? <Text style={styles.detailLabel}>{segment.countryName}</Text> : null}{segment.notes ? <Text style={styles.detailValue}>{segment.notes}</Text> : null}{!readOnly && <View style={styles.actionRow}><TouchableOpacity style={styles.smallButton} onPress={onEdit}><Text style={styles.smallButtonText}>✎</Text></TouchableOpacity><TouchableOpacity style={styles.deleteButton} onPress={onDelete}><Text style={styles.deleteButtonText}>×</Text></TouchableOpacity></View>}</View>;
}
...
export function TripDetailScreen() {
  ...
  const performTripDelete = async () => { if (deleting) return; setDeleting(true); try { await deleteTrip(trip.id); navigation.navigate("MyTrips"); } catch (error) { const firestoreError = error as { code?: string; message?: string }; console.error("Delete trip error", { code: firestoreError.code, message: firestoreError.message, error }); const denied = firestoreError.code === "permission-denied"; Alert.alert(t("trips.deleteFailedTitle"), t(denied ? "trips.permissionDeniedMessage" : "trips.deleteFailedMessage")); } finally { setDeleting(false); } };
