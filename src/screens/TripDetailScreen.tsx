...
export function TripDetailScreen() {
  ...
  const hasFlightSummary = Boolean(hasFlightLeg(trip.flightDetails?.outbound) || hasFlightLeg(trip.flightDetails?.return));
  const outboundReminder = trip.reminderPlan.find((item) => item.type === "outboundFlight");
  ...
  return <ScrollView style={styles.container} contentContainerStyle={[styles.content, { paddingTop: getScreenTopInset(insets.top), paddingBottom: getFloatingTabClearance(insets.bottom) }]} scrollIndicatorInsets={{ bottom: getScrollIndicatorBottomInset(insets.bottom) }}>
    ...
    {hasFlightSummary && <View style={styles.card}><Text style={styles.sectionTitle}>{t("flightAlerts.flightInfo")}</Text><FlightLegCard label={t("flightAlerts.outboundFlight")} leg={trip.flightDetails?.outbound} reminder={outboundReminder} t={t} /><FlightLegCard label={t("flightAlerts.returnFlight")} leg={trip.flightDetails?.return} reminder={returnReminder} t={t} /></View>}
