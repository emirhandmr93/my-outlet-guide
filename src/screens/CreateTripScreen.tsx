...
export function CreateTripScreen() {
  ...
  useEffect(() => {
    requireAuth({
      isLoggedIn,
      navigation,
      message: t("trips.authRequiredCreateMessage"),
    });
  ...
  function openTimePicker() {
    ...
    setDraftTime(next);
    setTimePickerTarget("returnTime");
  }
  ...
  function confirmDatePicker() {
    ...
    if (selected < todayAtMidnight()) {
      Alert.alert(
        ...
        t("createTrip.pastDateBlocked"),
      );
      return;
    }
    if (pickerTarget === "start") {
      setStartDate(selected);
      if (endDate && formatDate(endDate) < formatDate(selected))
        setEndDate(null);
    } else if (pickerTarget === "end") {
      if (startDate && formatDate(selected) < formatDate(startDate)) {
        Alert.alert(
          t("createTrip.validationTitle"),
          t("createTrip.endDateBeforeStart"),
        );
        return;
      }
      const previousEndDate = endDate ? formatDate(endDate) : null;
      const selectedEndDate = formatDate(selected);
      setEndDate(selected);
      if (
        !returnDepartureDate ||
        autoReturnFlightDateSource === previousEndDate
      ) {
        setReturnDepartureDate(selectedEndDate);
  ...
  async function saveTrip() {
    ...
    if (!tripName.trim())
      return Alert.alert(
        t("createTrip.validationTitle"),
        t("createTrip.tripNameRequired"),
      );
    if (!startDate || !endDate)
      return Alert.alert(
        t("createTrip.validationTitle"),
        t("createTrip.dateRangeRequired"),
      );
    ...
    if (startDate < todayAtMidnight() || endDate < todayAtMidnight())
      return Alert.alert(
        t("createTrip.validationTitle"),
        t("createTrip.pastDateBlocked"),
      );
  ...
  const pickerTitle =
    pickerTarget === "start"
      ? t("createTrip.selectStartDateTitle")
      : pickerTarget === "end"
        ? t("createTrip.selectEndDateTitle")
        : t("createTrip.returnFlightDatePickerTitle");
  ...
  return (
    ...
    >
          ...
          {(
            [
              [
                ...
                "createTrip.selectStartDate",
              ],
              [
                "end",
                endDate,
                "createTrip.endDate",
                "createTrip.selectEndDate",
              ],
            ] as const
          ).map(([target, value, label, placeholder]) => (
            <View style={styles.dateColumn} key={target}>
              <Text style={styles.label}>{t(label)}</Text>
              <TouchableOpacity
                style={[
                  styles.inputBox,
                  pickerTarget === target && styles.activeInputBox,
                ]}
                activeOpacity={0.76}
        ...
        >
          <View style={styles.headerText}>
            <Text style={styles.sectionTitle}>
              {t("createTrip.returnFlightReminderTitle")}
            </Text>
            <Text style={styles.helperText}>
              {t("createTrip.returnFlightReminderHelper")}
            </Text>
          </View>
          <Text style={styles.chevron}>{flightOpen ? "⌃" : "⌄"}</Text>
        </TouchableOpacity>
        {flightOpen ? (
          <>
            <Text style={styles.label}>{t("createTrip.returnFlightDate")}</Text>
            <TouchableOpacity
              style={[
                styles.inputBox,
                pickerTarget === "return" && styles.activeInputBox,
              ]}
              activeOpacity={0.76}
            ...
            <Text style={styles.label}>{t("createTrip.returnFlightTime")}</Text>
            <TouchableOpacity
              style={[
                styles.inputBox,
                timePickerTarget === "returnTime" && styles.activeInputBox,
              ]}
              activeOpacity={0.76}
            ...
            >
              ...
              >
                {returnDepartureTime || t("createTrip.selectTime")}
              </Text>
              <Text style={styles.chevron}>⌄</Text>
            </TouchableOpacity>
      ...
      <View style={styles.card}>
        <Text style={styles.sectionTitle}>
          {t("createTrip.routeInfoTitle")}
        </Text>
        <Text style={styles.helperText}>{t("createTrip.routeInfoBody")}</Text>
      </View>
      <TouchableOpacity
      ...
      >
        <Text style={styles.primaryButtonText}>
          {saving ? t("createTrip.saving") : t("createTrip.createCta")}
        </Text>
      </TouchableOpacity>
      <Modal
...
const styles = StyleSheet.create({
  ...
  helperText: { color: "#666666", lineHeight: 21, marginBottom: 12 },
  input: {
  ...
  activeInputBox: { borderColor: "#C9A227", backgroundColor: "#FFF9E8" },
  inputText: { color: "#0B1F3A", fontWeight: "800" },
  placeholderText: { color: "#8A8A8A", fontWeight: "700" },
  chevron: { color: "#C9A227", fontSize: 20, fontWeight: "900" },
