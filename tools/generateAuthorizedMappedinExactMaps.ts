...
const generatedFile = path.join(root, "src", "features", "premiumOutletMaps", "generatedMappedinExactMaps.ts");
const reportFile = path.join(root, "docs", "PREMIUM_MAP_MAPPEDIN_GENERATION_REPORT.json");

const mappedinOutletIds = [
  "bicester-village",
  "la-vallee-village",
  "la-roca-village",
  "las-rozas-village",
...
function polygonParts(feature: GeoFeature): Polygon[] {
  ...
  if (feature.geometry.type === "MultiPolygon" && Array.isArray(feature.geometry.coordinates)) {
    return feature.geometry.coordinates
      .map(asPolygon)
      .filter((value): value is Polygon => Boolean(value));
  }
  return [];
}
...
function findMvfRoot(outletId: string): string {
  ...
  if (directories.length !== 1) throw new Error(`${outletId}: expected one sanitized MVF bundle, found ${directories.length}`);
  return path.join(base, directories[0].name);
}

function sameLabel(): LocalizedLabel {
  return { en: "", tr: "", es: "", fr: "", de: "", ar: "", ru: "", zh: "" };
}

function localizedSourceLabel(value: string): LocalizedLabel {
  const labels = sameLabel();
  for (const key of Object.keys(labels) as Array<keyof LocalizedLabel>) labels[key] = value;
  return labels;
}
...
function loadFloors(mvfRoot: string): PremiumMapFloor[] {
  ...
  return sorted.map((feature, index) => {
    ...
    return {
      ...
      level,
      label: localizedSourceLabel(sourceName),
    };
  });
}
...
function loadNodes(mvfRoot: string): Map<string, GeoFeature> {
  ...
  const collection = readJson<GeoCollection>(path.join(mvfRoot, "node.geojson"));
  for (const feature of collection.features ?? []) {
    const id = feature.properties?.id;
...
function exactLocationsForBrand(brand: BrandLike, locations: LocationRecord[]): LocationRecord[] {
  const names = [brand.brandName, ...(brand.aliases ?? [])].map(normalize).filter(Boolean);
  const accepted = new Set(names);
  return locations.filter(location => accepted.has(normalize(location.name ?? "")));
}
...
for (const outletId of mappedinOutletIds) {
  ...
  for (const brand of brands) {
    const matches = exactLocationsForBrand(brand, tenantLocations);
  ...
  maps[outletId] = {
    ...
    source: {
      ...
      attribution: `Official ${source.operator} map data`,
    },
  };
...
const generated = `import type { PremiumOutletMap } from "./types";\n\n/**\n * AUTO-GENERATED from authorized, sanitized Mappedin evidence. Do not hand-edit.\n * Generated by tools/generateAuthorizedMappedinExactMaps.ts.\n */\nexport const generatedMappedinExactMaps: Record<string, PremiumOutletMap> = ${JSON.stringify(maps, null, 2)};\n`;
fs.writeFileSync(generatedFile, generated, "utf8");

const summary = {
  outletCount: mappedinOutletIds.length,
