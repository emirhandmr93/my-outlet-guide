...
for (const outletId of ["deepo-outlet-center", "venezia-mega-outlet"]) assert(!outletBrands.some((r)=>r.outletId===outletId), `${outletId} must have zero relations.`);
...
const changed=execFileSync("git",["diff","--name-only",`${mergeBase}...HEAD`],{encoding:"utf8"}).trim().split("\n").filter(Boolean); const allowed=new Set(["src/constants/outletBrands/turkey.ts",...brandFiles,"tools/checkTurkeyBrandCoverageStarCity.ts","tools/checkTurkeyBrandCoverageOlivium.ts","tools/checkTurkeyBrandCoverageViaport.ts","tools/checkTurkeyBrandCoverageIstanbulOptimum.ts","tools/checkTurkeyBrandCoverageIzmirOptimum.ts", "tools/checkTurkeyBrandCoverageViaport.ts","tools/checkTurkeyExpansion.ts","tools/checkTurkeyBasicMetadataBatchA.ts","tools/checkTurkeyBasicMetadataBatchB.ts","tools/checkTurkeyBrandCoverage212.ts"]); assert(changed.every((file)=>allowed.has(file)), "Changed file is outside permitted scope.");
