...
assert(starCityRelations.length === 102 && oliviumRelations.length === 94, "Expected 101 StarCity and preserved 94 Olivium relations.");
...
for (const outletId of ["212-outlet", "deepo-outlet-center", "venezia-mega-outlet"]) assert(!outletBrands.some((r)=>r.outletId===outletId), `${outletId} must have zero relations.`);
...
const changed=execFileSync("git",["diff","--name-only",`${mergeBase}...HEAD`],{encoding:"utf8"}).trim().split("\n").filter(Boolean); const allowed=new Set(["src/constants/outletBrands/turkey.ts",...brandFiles,"tools/checkTurkeyBrandCoverageStarCity.ts","tools/checkTurkeyBrandCoverageOlivium.ts","tools/checkTurkeyBrandCoverageIstanbulOptimum.ts","tools/checkTurkeyBrandCoverageIzmirOptimum.ts","tools/checkTurkeyBrandCoverageViaport.ts","tools/checkTurkeyExpansion.ts","tools/checkTurkeyBasicMetadataBatchA.ts","tools/checkTurkeyBasicMetadataBatchB.ts"]); assert(changed.every((file)=>allowed.has(file)), "Changed file is outside permitted scope.");
