...
const changed=execFileSync("git",["diff","--name-only",`${mergeBase}...HEAD`],{encoding:"utf8"}).trim().split("\n").filter(Boolean); const allowed=new Set(["src/constants/outletBrands/turkey.ts",...brandFiles,"tools/checkTurkeyBrandCoverageStarCity.ts","tools/checkTurkeyBrandCoverageOlivium.ts","tools/checkTurkeyExpansion.ts","tools/checkTurkeyBasicMetadataBatchA.ts","tools/checkTurkeyBasicMetadataBatchB.ts"]); assert(changed.every((file)=>allowed.has(file)), "Changed file is outside permitted scope.");
