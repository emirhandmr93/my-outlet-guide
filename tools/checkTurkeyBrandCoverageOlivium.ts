...
for (const outlet of turkeyOutlets) if (outlet.outletId !== oliviumOutletId && outlet.outletId !== "starcity-outlet") assert(!outletBrands.some((relation) => relation.outletId === outlet.outletId), `${outlet.outletId} must have zero brand relations.`);
...
const allowedFiles = new Set([
  ...
  ...brandSourceFiles,
  "tools/checkTurkeyBrandCoverageOlivium.ts", "tools/checkTurkeyBrandCoverageStarCity.ts", "tools/checkTurkeyExpansion.ts",
