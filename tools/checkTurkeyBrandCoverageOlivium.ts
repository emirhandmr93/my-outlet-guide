...
for (const outlet of turkeyOutlets) if (outlet.outletId !== oliviumOutletId && outlet.outletId !== "starcity-outlet" && outlet.outletId !== "optimum-premium-outlet-istanbul" && outlet.outletId !== "izmir-optimum") assert(!outletBrands.some((relation) => relation.outletId === outlet.outletId), `${outlet.outletId} must have zero brand relations.`);
...
const allowedFiles = new Set([
  ...
  ...brandSourceFiles,
  "tools/checkTurkeyBrandCoverageOlivium.ts", "tools/checkTurkeyBrandCoverageStarCity.ts", "tools/checkTurkeyBrandCoverageIstanbulOptimum.ts", "tools/checkTurkeyBrandCoverageIzmirOptimum.ts", "tools/checkTurkeyExpansion.ts",
