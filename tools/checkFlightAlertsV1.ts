...
assert(screens.includes("hasAnyValue(outboundValues)") && screens.includes("hasAnyValue(returnValues)"), "flight fields must be optional and validated only when entered");
assert(services.includes("outbound?.departureDate") && services.includes("return?.departureDate") && services.includes("outboundFlight") && services.includes("returnFlight"), "flight reminders must derive from valid flight dates");
...
assert(screens.includes("trip.flightDetails?.outbound") && screens.includes("trip.flightDetails?.return"), "Trip Detail must render saved trip flight fields");
