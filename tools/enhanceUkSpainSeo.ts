...
const TARGET_COUNTRIES = ["united-kingdom", "spain"] as const;
const TARGET_CITIES = ["london", "madrid", "barcelona"] as const;
...
const UK_KEYWORD: Record<TranslationLanguage, string> = {
  en: "UK",
  tr: "İngiltere",
  es: "Reino Unido",
  fr: "Royaume-Uni",
...
const COUNTRY_PRIMARY_CITY: Record<TargetCountry, string> = {
  "united-kingdom": "london",
  spain: "madrid",
};

const COUNTRY_TEMPLATES: Record<TranslationLanguage, CountryCopy> = {
  en: {
    title: "{country} Outlets: {city} Outlet Guide | My Outlet Guide",
    description: "Explore outlet shopping in {country}. Compare {outletCount} outlet destinations across {cityCount} shopping cities, including options linked with {city}, plus brands, transport and Tax Free guidance.",
    heading: "{country} outlet shopping guide",
    introHeading: "Outlet shopping in {country}",
    summary: "Compare {outletCount} outlet destinations across {cityCount} shopping cities in {country}. Open each city or outlet guide for brands, visit details and available transportation information.",
...
const CITY_TEMPLATES: Record<TranslationLanguage, CityCopy> = {
  ...
  de: { title:"Outlets in {city}: Shopping-Guide | My Outlet Guide", description:"Entdecken Sie Outlets für {city}, {country}. Vergleichen Sie {outletCount} Ziele, Marken, Anreise-Guides und Tax-Free-Informationen.", heading:"Outlet-Shopping-Guide für {city}", introHeading:"Outlet-Shopping in {city}", summary:"Vergleichen Sie {outletCount} Outlet-Ziele für {city}. Prüfen Sie Marken und Besuchsinformationen und nutzen Sie verfügbare Anreise-Guides für Ihre Planung.", outletsHeading:"Outlet-Ziele für {city}", transportHeading:"Anreise zu Outlet-Zielen bei {city}", planningHeading:"Outlet-Tag in {city} planen", planningText:"Vergleichen Sie Standort, Anreise und Tax-Free-Informationen, bevor Sie das passende Outlet für Ihre Route auswählen.", countryGuideLabel:"Outlet-Guide für {country}", taxFreeLabel:"Tax-Free-Rechner", transportLabel:"Anreise-Guide für {outlet}" },
  ar: { title:"أوت لت {city}: دليل التسوق | My Outlet Guide", description:"اكتشف وجهات الأوت لت لـ {city} في {country}. قارن {outletCount} وجهة والعلامات وأدلة النقل ومعلومات Tax Free.", heading:"دليل تسوق الأوت لت في {city}", introHeading:"تسوق الأوت لت في {city}", summary:"قارن بين {outletCount} وجهة أوت لت مدرجة لـ {city}. راجع العلامات ومعلومات الزيارة واستخدم أدلة النقل المتاحة لتخطيط يوم التسوق.", outletsHeading:"وجهات الأوت لت لـ {city}", transportHeading:"كيفية الوصول إلى أوت لت {city}", planningHeading:"خطط لرحلة أوت لت في {city}", planningText:"قارن الموقع والنقل ومعلومات Tax Free قبل اختيار الوجهة الأنسب لمسارك.", countryGuideLabel:"دليل أوت لت {country}", taxFreeLabel:"حاسبة Tax Free", transportLabel:"دليل النقل إلى {outlet}" },
  ru: { title:"Аутлеты {city}: гид по шопингу | My Outlet Guide", description:"Изучайте аутлеты для {city}, {country}. Сравните {outletCount} направлений, бренды, транспортные гиды и информацию Tax Free.", heading:"Гид по аутлетам: {city}", introHeading:"Аутлет-шопинг в {city}", summary:"Сравните {outletCount} аутлет-направлений для {city}. Проверяйте бренды и информацию о посещении и используйте доступные транспортные гиды для планирования.", outletsHeading:"Аутлеты для {city}", transportHeading:"Как добраться до аутлетов {city}", planningHeading:"Спланируйте поездку по аутлетам {city}", planningText:"Сравните расположение, транспорт и информацию Tax Free перед выбором аутлета для маршрута.", countryGuideLabel:"Гид по аутлетам: {country}", taxFreeLabel:"Калькулятор Tax Free", transportLabel:"Транспортный гид: {outlet}" },
  zh: { title:"{city}奥特莱斯：购物指南 | My Outlet Guide", description:"探索{city}（{country}）相关奥特莱斯，对比 {outletCount} 个目的地、品牌、交通指南和 Tax Free 退税信息。", heading:"{city}奥特莱斯购物指南", introHeading:"{city}奥特莱斯购物", summary:"对比为{city}列出的 {outletCount} 个奥特莱斯目的地。查看品牌和到访信息，并利用现有交通指南规划购物行程。", outletsHeading:"{city}奥特莱斯目的地", transportHeading:"如何前往{city}奥特莱斯", planningHeading:"规划{city}奥特莱斯行程", planningText:"选择最适合行程的奥特莱斯前，先比较位置、交通和 Tax Free 退税信息。", countryGuideLabel:"{country}奥特莱斯指南", taxFreeLabel:"Tax Free 退税计算器", transportLabel:"{outlet}交通指南" },
};

function escapeHtml(value: string) { return value.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"); }
function fill(value: string, values: Record<string,string|number>) { return Object.entries(values).reduce((result,[key,replacement])=>result.replaceAll(`{${key}}`,String(replacement)),value); }
function keywordCountry(countryId: string, language: TranslationLanguage) { return countryId === "united-kingdom" ? UK_KEYWORD[language] : formatCountryDisplayName(countryId, language); }
function replaceMeta(html:string,title:string,description:string) { const t=escapeHtml(title),d=escapeHtml(description); return html.replace(/<title>[\s\S]*?<\/title>/i,`<title>${t}</title>`).replace(/<meta name="description" content="[^"]*">/i,`<meta name="description" content="${d}">`).replace(/<meta property="og:title" content="[^"]*">/i,`<meta property="og:title" content="${t}">`).replace(/<meta property="og:description" content="[^"]*">/i,`<meta property="og:description" content="${d}">`).replace(/<meta name="twitter:title" content="[^"]*">/i,`<meta name="twitter:title" content="${t}">`).replace(/<meta name="twitter:description" content="[^"]*">/i,`<meta name="twitter:description" content="${d}">`); }
function updateStructuredData(html:string,pageId:string,title:string,description:string) { return html.replace(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/i,(match,raw:string)=>{ try { const data=JSON.parse(raw) as {"@graph"?:Record<string,unknown>[]}; const webpage=(data["@graph"]??[]).find(item=>item["@id"]===pageId); if (!webpage) return match; webpage.name=title; webpage.description=description; return `<script type="application/ld+json">${JSON.stringify(data).replace(/</g,"\\u003c")}</script>`; } catch { return match; } }); }
function renderTemplate<T extends Record<string,string>>(template:T,values:Record<string,string|number>):T { return Object.fromEntries(Object.entries(template).map(([key,value])=>[key,fill(value,values)])) as T; }
...
async function enhanceCountry(language:TranslationLanguage,countryId:TargetCountry) {
  ...
  const values={country:keywordCountry(countryId,language),city:formatCityDisplayName(COUNTRY_PRIMARY_CITY[countryId],language),outletCount:countryOutlets.length,cityCount:cityIds.length};
  const copy=renderTemplate(COUNTRY_TEMPLATES[language],values);
  ...
  const file=join(DIST,language,"country",`${countryId}.html`); let html=await readFile(file,"utf8"); html=replaceMeta(html,copy.title,copy.description); html=updateStructuredData(html,`${WEB_SEO_ORIGIN}/${language}/country/${countryId}#webpage`,copy.title,copy.description); html=html.replace(/(<main data-web-fallback="true"[^>]*>)<h1>[\s\S]*?<\/h1><p>[\s\S]*?<\/p>/i,`$1<h1>${escapeHtml(copy.heading)}</h1><p>${escapeHtml(copy.description)}</p>${section}`); await writeFile(file,html);
}

async function enhanceCity(language:TranslationLanguage,cityId:TargetCity) {
  const city=cities.find(item=>item.cityId===cityId); if (!city) throw new Error(`Unknown city ${cityId}`);
  const cityOutlets=outlets.filter(outlet=>outlet.cityId===cityId&&isWebSeoPublicOutlet(outlet)).sort((a,b)=>a.name.localeCompare(b.name)); if (!cityOutlets.length) throw new Error(`No public outlets for ${cityId}`);
  const values={city:formatCityDisplayName(cityId,language),country:keywordCountry(city.countryId,language),outletCount:cityOutlets.length}; const copy=renderTemplate(CITY_TEMPLATES[language],values); const href=(path:string)=>`${WEB_SEO_ORIGIN}/${language}/${path}`;
  const outletList=cityOutlets.map(outlet=>`<li><a href="${href(`outlet/${outlet.outletId}`)}">${escapeHtml(outlet.name)}</a></li>`).join("");
  const transportLinks=cityOutlets.filter(outlet=>hasWebSeoTransportation(outlet.outletId)).map(outlet=>`<li><a href="${href(`transportation/${outlet.outletId}`)}">${escapeHtml(fill(copy.transportLabel,{outlet:outlet.name}))}</a></li>`).join("");
  const transportSection=transportLinks?`<h2>${escapeHtml(copy.transportHeading)}</h2><ul>${transportLinks}</ul>`:"";
  const section=`<section data-priority-city-seo="${cityId}"><h2>${escapeHtml(copy.introHeading)}</h2><p>${escapeHtml(copy.summary)}</p><h2>${escapeHtml(copy.outletsHeading)}</h2><ul>${outletList}</ul>${transportSection}<h2>${escapeHtml(copy.planningHeading)}</h2><p>${escapeHtml(copy.planningText)}</p><p><a href="${href(`country/${city.countryId}`)}">${escapeHtml(fill(copy.countryGuideLabel,{country:keywordCountry(city.countryId,language)}))}</a> · <a href="${href("calculator/tax-free")}">${escapeHtml(copy.taxFreeLabel)}</a></p></section>`;
  const file=join(DIST,language,"city",`${cityId}.html`); let html=await readFile(file,"utf8"); html=replaceMeta(html,copy.title,copy.description); html=updateStructuredData(html,`${WEB_SEO_ORIGIN}/${language}/city/${cityId}#webpage`,copy.title,copy.description); html=html.replace(/(<main data-web-fallback="true"[^>]*>)<h1>[\s\S]*?<\/h1><p>[\s\S]*?<\/p>/i,`$1<h1>${escapeHtml(copy.heading)}</h1><p>${escapeHtml(copy.description)}</p>${section}`); await writeFile(file,html);
}
...
async function main() {
  ...
  console.log("enhanceUkSpainSeo: enhanced UK, Spain, London, Madrid and Barcelona landing pages in 8 languages.");
}
