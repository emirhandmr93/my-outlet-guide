...
Options:
  --evaluation-date <YYYY-MM-DD>  Seed/preflight require the current UTC date; defaults to today UTC
...
function validateRuntime(runtime, uid, requireSingleton = false) {
  ...
  if (requireSingleton && (runtime.testUserIds.length !== 1 || runtime.testUserIds[0] !== uid)) fail(`Runtime isolation conflict count: ${runtime.testUserIds.length}.`);
  return runtime.testUserIds.length;
}
...
function documentPath(document) {
  if (!document || typeof document !== "object" || typeof document.name !== "string" || !document.fields || typeof document.fields !== "object") fail("Alert listing returned a malformed document envelope.");
  const marker = "/documents/", offset = document.name.indexOf(marker); if (offset < 0) fail("Alert listing returned a malformed document path.");
  try { return decodeURIComponent(document.name.slice(offset + marker.length)); } catch { fail("Alert listing returned a malformed encoded path."); }
}
export function classifyIsolationAlert(path, value, evaluationDate) {
  const parts = typeof path === "string" ? path.split("/") : [];
  if (parts.length !== 4 || parts[0] !== "flightDealPreferences" || parts[2] !== "alerts" || !validSegment(parts[1]) || !validSegment(parts[3]) || !value || typeof value !== "object") return null;
  const passengers = Number.isInteger(value.adults) && value.adults >= 1 && value.adults <= 9 && Number.isInteger(value.children) && value.children >= 0 && value.children <= 8 && Number.isInteger(value.infants) && value.infants >= 0 && value.infants <= 9 && value.adults + value.children <= 9 && value.infants <= value.adults;
  const thresholds = Array.isArray(value.selectedThresholds) && value.selectedThresholds.length > 0 && value.selectedThresholds.every(item => item === 15 || item === 30 || item === 45);
  if (value.schemaVersion !== 2 || value.userId !== parts[1] || value.alertId !== parts[3] || value.queryKey !== value.alertId || value.active !== true || value.providerStatus !== "pending_provider" || value.currency !== "EUR" || !passengers || !thresholds || !isDate(value.departDate) || value.departDate < evaluationDate) return null;
  try { return { path, userId: parts[1], documentId: parts[3], providerQueryKey: providerKey(value) }; } catch { return null; }
}
async function providerIsolation(db, context, required = true) {
  const parent = `flightDealPreferences/${context.alert.userId}`, documents = await db.list(parent, "alerts");
  const active = documents.map(document => { const path = documentPath(document); if (!path.startsWith(`${parent}/alerts/`) || path.split("/").length !== 4) fail("Alert listing returned an incomplete or foreign path."); return classifyIsolationAlert(path, data(document), context.evaluationDate); }).filter(Boolean);
  const owners = active.filter(item => item.providerQueryKey === context.providerQueryKey);
  const targetPath = `${parent}/alerts/${context.alert.alertId}`;
  const valid = owners.length === 1 && owners[0].path === targetPath;
  if (required && !valid) fail(`Provider-query isolation conflict count: ${owners.length}.`);
  return { count: owners.length, valid };
}
async function coreContext(db, args, { requireSingleton = false, requireCurrentDate = false } = {}) {
  const evaluationDate = args["evaluation-date"] ?? new Date().toISOString().slice(0, 10), today = new Date().toISOString().slice(0, 10);
  if (!isDate(evaluationDate)) fail("Evaluation date must be a real YYYY-MM-DD calendar date.");
  if (requireCurrentDate && evaluationDate !== today) fail("Preflight and seed require the current UTC evaluation date.");
  ...
  const runtimeAllowlistCount = validateRuntime(data(runtimeDoc), uid, requireSingleton);
  ...
  return { runtimeDoc, runtimeAllowlistCount, alertDoc, alert, evaluationDate, providerQueryKey: key, eventId: id };
}
async function pushEligibility(db, uid, required) {
  const settings = await db.get(`userNotificationSettings/${uid}`), tokens = await db.list(`userNotificationSettings/${uid}`, "tokens"), count = eligibleTokens(tokens, uid);
...
export async function preflight(db, args, quiet = false, requirePush = true) {
  const strictStart = args.action === "preflight" || args.action === "seed";
  const context = await coreContext(db, args, { requireSingleton: strictStart, requireCurrentDate: strictStart });
  const isolation = await providerIsolation(db, context, strictStart), providerOwnerCount = isolation.count;
  const eligibility = requirePush ? await pushEligibility(db, args["user-id"], true) : { enabled: false, count: 0 };
  if (!quiet) { log("action", args.action); log("project", PROJECT); log("user", mask(args["user-id"])); log("alert", mask(args["alert-id"])); log("evaluation date", context.evaluationDate); log("runtime allowlist count", context.runtimeAllowlistCount); log("provider-key owner count", providerOwnerCount); log("provider query key", mask(context.providerQueryKey)); log("event ID", mask(context.eventId)); log("eligible token count", eligibility.count); log("expected discount", EXPECTED_DISCOUNT.toFixed(2)); log("status", "preflight_passed"); }
  return { ...context, providerOwnerCount, providerIsolationValid: isolation.valid, eligibleTokenCount: eligibility.count };
}
...
function priorEvaluationIdentityMatches(value, context) { return value?.schemaVersion === 1 && value.userId === context.alert.userId && value.alertId === context.alert.alertId && value.queryKey === context.alert.queryKey && value.providerQueryKey === context.providerQueryKey && profileMatches(value, context.alert); }
...
export async function seed(db, args) {
  ...
  if (evaluation && !priorEvaluationIdentityMatches(data(evaluation), context)) fail("Existing evaluation identity conflict; seed refused before backup.");
  ...
  const backup = { schemaVersion: 3, projectId: PROJECT, maskedMetadata: { user: mask(uid), alert: mask(alertId), providerQueryKey: mask(context.providerQueryKey), eventId: mask(context.eventId) }, userId: uid, alertId, providerQueryKey: context.providerQueryKey, eventId: context.eventId, evaluationDate: context.evaluationDate, currentUtcEvaluationDate: context.evaluationDate, syntheticTestRunId: runId, syntheticDateIds: dates, existence: { currentSnapshot: Boolean(current), evaluation: Boolean(evaluation) }, evaluationExistedBeforeSeed: Boolean(evaluation), evaluationResetBySeed: Boolean(evaluation), singletonRuntimeConfirmed: context.runtimeAllowlistCount === 1, providerIsolationResultCount: context.providerOwnerCount, originalStateDocument: rawClone(state), originalCurrentSnapshotDocument: current ? rawClone(current) : null, originalEvaluationDocument: evaluation ? rawClone(evaluation) : null, originalParentTestFields: rawClone(originalTestFields), expectedRawSyntheticParentPatch: encodeFields(parentPatch), expectedRawSyntheticSnapshotFields: rawClone(snapshotFields), createdAt: new Date().toISOString() };
  ...
  if (evaluation) writes.push(db.delete(evaluationPath, { updateTime: evaluation.updateTime }));
  await db.commit(writes);
  const [savedState, savedEvaluation, ...saved] = await Promise.all([db.get(statePath), db.get(evaluationPath), ...dates.map(date => db.get(`${statePath}/dailySnapshots/${date}`))]);
  const savedData = data(savedState), syntheticValid = savedState && !savedEvaluation && savedData.firstSnapshotDate === dates[0] && savedData.syntheticTestRunId === runId && rawEqual(withoutTestFields(savedState.fields), withoutTestFields(state.fields)) && saved.every((document, index) => document && rawEqual(document.fields, snapshotFields[index]));
  if (!syntheticValid) fail(`Seed verification failed; do not invoke Scheduler and run immediate cleanup using backup ${backupPath}.`);
  try { const isolated = await coreContext(db, args, { requireSingleton: true, requireCurrentDate: true }); await providerIsolation(db, isolated, true); }
  catch { fail(`Post-seed isolation verification failed; do not invoke Scheduler and run immediate cleanup using backup ${backupPath}.`); }
  log("action", "seed"); log("project", PROJECT); log("evaluation date", context.evaluationDate); log("provider query key", mask(context.providerQueryKey)); log("event ID", mask(context.eventId)); log("synthetic snapshot count", 14); log("evaluation baseline", evaluation ? "reset" : "absent"); log("expected discount", EXPECTED_DISCOUNT.toFixed(2)); log("backup path", backupPath); log("test run ID", runId); log("status", "seed_verified");
}
...
export async function verify(db, args) {
  ...
  const synthetic = context.runtimeAllowlistCount === 1 && context.providerIsolationValid && validUuid(runId) && parentMatches(stateData, context) && stateData.firstSnapshotDate === dates[0] && snapshots.every((document, index) => document && snapshotMatches(data(document), context, dates[index], index === 13 ? 80 : 100, runId));
...
function validateBackup(backup, args, context) { const uid = args["user-id"], alertId = args["alert-id"]; if (!backup || backup.schemaVersion !== 3 || backup.projectId !== PROJECT || backup.userId !== uid || backup.alertId !== alertId || backup.providerQueryKey !== context.providerQueryKey || backup.eventId !== eventId(uid, alertId, backup.evaluationDate) || backup.eventId !== context.eventId || !validUuid(backup.syntheticTestRunId) || !Array.isArray(backup.syntheticDateIds) || !rawEqual(backup.syntheticDateIds, syntheticDates(backup.evaluationDate)) || backup.currentUtcEvaluationDate !== backup.evaluationDate || backup.evaluationExistedBeforeSeed !== Boolean(backup.originalEvaluationDocument) || backup.evaluationResetBySeed !== backup.evaluationExistedBeforeSeed || backup.singletonRuntimeConfirmed !== true || backup.providerIsolationResultCount !== 1 || !backup.originalStateDocument || !backup.originalParentTestFields || !Array.isArray(backup.expectedRawSyntheticSnapshotFields) || backup.expectedRawSyntheticSnapshotFields.length !== 14 || typeof backup.createdAt !== "string") fail("Backup validation or supplied path identity validation failed."); }
...
export async function cleanup(db, args) {
  ...
  const context = await coreContext(db, args, { requireSingleton: false, requireCurrentDate: false }); validateBackup(backup, args, context);
  ...
  const evaluationState = rawDocumentEqual(evaluation, originalEvaluation) ? "EXACT_ORIGINAL" : !evaluation ? "ABSENT_AFTER_SEED" : evaluationMatches(data(evaluation), context) ? "STRICT_SYNTHETIC" : "CONFLICT";
  if (evaluationState === "CONFLICT") fail("Evaluation conflicts with the synthetic test; cleanup stopped before writes.");
  ...
  if (evaluationState === "ABSENT_AFTER_SEED" && originalEvaluation) writes.push(db.update(evaluationPath, rawClone(originalEvaluation.fields), { exists: false }));
  if (evaluationState === "STRICT_SYNTHETIC") writes.push(originalEvaluation ? db.update(evaluationPath, rawClone(originalEvaluation.fields), { updateTime: evaluation.updateTime }) : db.delete(evaluationPath, { updateTime: evaluation.updateTime }));
  await db.commit(writes);
