...
assert(countries.length === 35 && countries.filter(({ taxFreeStatus }) => taxFreeStatus === "available").length === 31 && countries.filter(({ taxFreeStatus }) => taxFreeStatus === "not_available").length === 4, "Coverage remains 35 countries, 31 available, and 4 not available.");
