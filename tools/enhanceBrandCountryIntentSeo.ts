...
const COPY: Record<TranslationLanguage, Copy> = {
  en: {
    countryLabel: "{brand} outlets in {country}",
    taxFreeLabel: "Tax Free planning for {brand} outlet shopping",
  },
  tr: {
    countryLabel: "{country} içindeki {brand} outlet mağazaları",
    taxFreeLabel: "{brand} outlet alışverişi için Tax Free planlama",
  },
  es: {
    countryLabel: "Outlets de {brand} en {country}",
    taxFreeLabel: "Planificación Tax Free para compras outlet de {brand}",
  },
  fr: {
    countryLabel: "Outlets {brand} en {country}",
    taxFreeLabel: "Planification Tax Free pour le shopping outlet {brand}",
  },
  de: {
    countryLabel: "{brand} Outlets in {country}",
    taxFreeLabel: "Tax-Free-Planung für {brand} Outlet-Shopping",
  },
  ar: {
    countryLabel: "أوت لت {brand} في {country}",
    taxFreeLabel: "تخطيط Tax Free لتسوق أوت لت {brand}",
  },
  ru: {
    countryLabel: "Аутлеты {brand} в {country}",
    taxFreeLabel: "Tax Free для покупок в аутлетах {brand}",
  },
  zh: {
    countryLabel: "{country}{brand}奥特莱斯门店",
    taxFreeLabel: "{brand}奥特莱斯购物 Tax Free 退税规划",
  },
};
...
async function enhanceBrand(language: TranslationLanguage, brand: (typeof brands)[number]) {
  ...
  const sectionPattern = new RegExp(
    `<section data-brand-seo="${escapeRegExp(brand.brandId)}"[^>]*>[\\s\\S]*?<\\/section>`,
    "i",
  );
  const sectionMatch = html.match(sectionPattern);
  if (!sectionMatch) throw new Error(`${language} brand ${brand.brandId}: SEO section not found`);

  let section = sectionMatch[0].replace(
    `<section data-brand-seo="${brand.brandId}">`,
    `<section data-brand-seo="${brand.brandId}" data-brand-country-intent="${brand.brandId}">`,
  );

  for (const countryId of countryIds.slice(0, MAX_COUNTRY_LINKS)) {
    const country = formatCountryDisplayName(countryId, language);
    const label = fill(COPY[language].countryLabel, {
      brand: brand.brandName,
      country,
    });
    section = replaceAnchorText(
      section,
...
async function main() {
  for (const language of WEB_SEO_LANGUAGES) {
    await inBatches(indexableBrands, (brand) => enhanceBrand(language, brand));
    console.log(
      `enhanceBrandCountryIntentSeo: completed ${language} (${indexableBrands.length} brands).`,
    );
  }

  console.log(
    `enhanceBrandCountryIntentSeo: localized brand + country + Tax Free anchor intent in ${WEB_SEO_LANGUAGES.length} languages.`,
  );
}
