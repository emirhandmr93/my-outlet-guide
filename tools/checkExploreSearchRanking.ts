...
const exploreScreenSource = readFileSync("src/screens/ExploreScreen.tsx", "utf8");
...
assert(
  exploreScreenSource.includes('ImageBackground') &&
    ...
    exploreScreenSource.includes('../../assets/explore/explore-hero-premium.png'),
...
assert(formatCityDisplayName("milan", "tr") === "Milano", "Milan should display as Milano in Turkish");
...
assert(formatCityDisplayName("naples", "tr") === "Napoli", "Naples should display as Napoli in Turkish");
assert(formatCityDisplayName("cologne", "tr") === "Köln", "Cologne should display as Köln in Turkish");
...
const localizedCountryQueries = [
  ["fransa", "france"], ["almanya", "germany"], ["italya", "italy"], ["birleşik krallık", "united-kingdom"],
  ["france", "france"], ["germany", "germany"], ["italy", "italy"], ["united kingdom", "united-kingdom"],
  ["francia", "france"], ["alemania", "germany"], ["italia", "italy"], ["reino unido", "united-kingdom"],
  ["allemagne", "germany"], ["italie", "italy"], ["royaume-uni", "united-kingdom"],
  ["frankreich", "france"], ["deutschland", "germany"], ["italien", "italy"], ["vereinigtes königreich", "united-kingdom"],
  ["франция", "france"], ["германия", "germany"], ["италия", "italy"], ["великобритания", "united-kingdom"],
  ["فرنسا", "france"], ["ألمانيا", "germany"], ["إيطاليا", "italy"], ["المملكة المتحدة", "united-kingdom"],
  ["法国", "france"], ["德国", "germany"], ["意大利", "italy"], ["英国", "united-kingdom"],
] as const;
...
const translationValues = Array.from(
  readFileSync("src/translations/translations.ts", "utf8").matchAll(/"[^"]+"\s*:\s*"([^"]*)"/g),
).map((match) => match[1]);
const noUserFacingAlias = translationValues.every((value) => !/\balias(?:es)?\b/i.test(value));
assert(noUserFacingAlias, "Explore UI and translations should not expose user-facing alias wording");
