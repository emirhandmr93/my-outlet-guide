...
assert(files.rules.includes("affectedKeys().hasOnly(['status', 'updatedAt', 'moderationNote', 'moderatedBy', 'moderatedAt'])") && files.rules.includes("request.resource.data.diff(resource.data).affectedKeys().hasOnly(['status', 'updatedAt'])"), "Firestore rules allow only narrow moderation fields");
