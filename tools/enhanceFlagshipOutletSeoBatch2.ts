...
function copyFor(language: TranslationLanguage, outlet: string, city: string, country: string): Copy {
  const values = { outlet, city, country };
  const fill = (text: string) => Object.entries(values).reduce((result, [key, value]) => result.replaceAll(`{${key}}`, value), text);
  const templates: Record<TranslationLanguage, Copy> = {
    en: {
      ...
      summary: "Compare selected brand pages for {outlet}, review transportation information and connect your visit with the {city} and {country} outlet guides.",
      ...
      planningText: "Check outlet details, transportation and Tax Free information before you travel.",
    ...
    tr: {
      ...
      description: "{city}, {country} yakınındaki {outlet} ziyaretinizi planlayın. Markaları, ulaşım rehberini ve Tax Free bilgilerini inceleyin.",
      heading: "{outlet} outlet alışveriş rehberi",
      introHeading: "{outlet} ziyaretinizi planlayın",
      summary: "{outlet} için seçili marka sayfalarını karşılaştırın, ulaşım bilgilerini inceleyin ve ziyaretinizi {city} ile {country} outlet rehberleriyle planlayın.",
      ...
      planningText: "Seyahatten önce outlet detaylarını, ulaşım rehberini ve Tax Free bilgilerini kontrol edin.",
    ...
    es: {
      ...
      description: "Planifica tu visita a {outlet}, cerca de {city}, {country}. Consulta marcas, transporte e información Tax Free antes de comprar.",
      heading: "Guía de compras outlet de {outlet}",
      introHeading: "Planifica tu visita a {outlet}",
      summary: "Compara páginas de marcas de {outlet}, revisa el transporte y conecta tu visita con las guías outlet de {city} y {country}.",
      ...
      planningText: "Revisa los detalles del outlet, el transporte y la información Tax Free antes del viaje.",
    ...
    fr: {
      ...
      description: "Planifiez votre visite à {outlet}, près de {city}, {country}. Consultez les marques, le transport et les informations Tax Free.",
      heading: "Guide shopping outlet de {outlet}",
      introHeading: "Planifier une visite à {outlet}",
      summary: "Comparez les pages marques de {outlet}, consultez les transports et reliez votre visite aux guides outlet de {city} et {country}.",
      ...
      planningText: "Consultez les détails de l'outlet, les transports et les informations Tax Free avant votre déplacement.",
    ...
    de: {
      ...
      description: "Planen Sie Ihren Besuch bei {outlet} nahe {city}, {country}. Prüfen Sie Marken, Anreise und Tax-Free-Informationen.",
      heading: "Outlet-Shopping-Guide für {outlet}",
      introHeading: "Besuch bei {outlet} planen",
      summary: "Vergleichen Sie Marken-Seiten für {outlet}, prüfen Sie die Anreise und verbinden Sie den Besuch mit den Outlet-Guides für {city} und {country}.",
      ...
      planningText: "Prüfen Sie vor der Reise Outlet-Details, Anreise- und Tax-Free-Informationen.",
    ...
    ar: {
      ...
      description: "خطط لزيارة {outlet} قرب {city} في {country}. راجع العلامات والنقل ومعلومات Tax Free قبل التسوق.",
      heading: "دليل التسوق في {outlet}",
      introHeading: "خطط لزيارة {outlet}",
      summary: "قارن صفحات العلامات في {outlet} وراجع النقل واربط الزيارة بأدلة الأوت لت في {city} و{country}.",
      ...
      planningText: "راجع تفاصيل الأوت لت والنقل ومعلومات Tax Free قبل السفر.",
    ...
    ru: {
      ...
      description: "Спланируйте посещение {outlet} рядом с {city}, {country}. Изучите бренды, транспорт и информацию Tax Free.",
      heading: "Гид по аутлету {outlet}",
      introHeading: "Спланируйте посещение {outlet}",
      summary: "Сравните страницы брендов {outlet}, проверьте транспорт и свяжите поездку с гидами по аутлетам {city} и {country}.",
      ...
      planningText: "Перед поездкой проверьте сведения об аутлете, транспорт и информацию Tax Free.",
    ...
    zh: {
      ...
      description: "规划前往{country}{city}附近的{outlet}。购物前查看品牌、交通和 Tax Free 退税信息。",
      heading: "{outlet}奥特莱斯购物指南",
      introHeading: "规划{outlet}行程",
      summary: "对比{outlet}品牌页面、查看交通信息，并结合{city}和{country}奥特莱斯指南规划购物行程。",
      brandsHeading: "{outlet}品牌",
      brandsIntro: "查看与{outlet}关联的精选活跃品牌页面。",
      planningHeading: "规划{outlet}购物行程",
      planningText: "出发前查看奥特莱斯详情、交通和 Tax Free 退税信息。",
      transportLabel: "交通指南",
  ...
  return Object.fromEntries(Object.entries(templates[language]).map(([key, value]) => [key, fill(value)])) as Copy;
}
...
async function enhanceOutlet(language: TranslationLanguage, outletId: TargetOutlet) {
  ...
  const copy = copyFor(language, outlet.name, cityName, countryName);
  ...
  const brandList = selectedBrands(outletId)
    ...
    .join("");
  const brandSection = brandList ? `<h2>${escapeHtml(copy.brandsHeading)}</h2><p>${escapeHtml(copy.brandsIntro)}</p><ul>${brandList}</ul>` : "";
  const transportLink = hasWebSeoTransportation(outletId) ? `<a href="${href(`transportation/${outletId}`)}">${escapeHtml(copy.transportLabel)}</a> · ` : "";
  ...
  const section = `<section data-flagship-outlet-seo-batch2="${outletId}"><h2>${escapeHtml(copy.introHeading)}</h2><p>${escapeHtml(copy.summary)}</p>${brandSection}<h2>${escapeHtml(copy.planningHeading)}</h2><p>${escapeHtml(copy.planningText)}</p><p>${transportLink}${cityLink} · ${countryLink} · ${taxFreeLink}</p></section>`;
...
async function main() {
  ...
  console.log("enhanceFlagshipOutletSeoBatch2: enhanced 5 additional flagship outlet landing pages in 8 languages.");
}
