...
const brandCategoryRows = (outletId: string) => { const related = outletBrands.filter((x: any) => x.outletId === outletId && x.relationStatus === "active"); const grouped = new Map<string, any[]>(); for (const rel of related) { const brand = (brands as any[]).find((b) => b.brandId === rel.brandId); if (!brand) continue; const id = brand?.luxuryLevel === "luxury" ? "luxury" : brand?.categoryId || "fashion"; grouped.set(id, [...(grouped.get(id) || []), brand]); } const total = related.length; const rows = Object.entries(categoryLabels).map(([id, [icon, label]]) => ({ id, icon, label, items: (grouped.get(id) || []).sort((a,b) => a.brandName.localeCompare(b.brandName, "tr")) })).filter((row) => row.items.length > 0); return { total, html: rows.map((row, index) => `<div class="app-brand-category" data-brand-category><button class="app-list-card app-category-row" type="button" data-brand-category-toggle><b>${row.icon}</b><span>${row.label}<small>${row.items.length} marka</small></span><i>›</i></button><div class="app-brand-chip-list" ${index ? "hidden" : ""}>${row.items.map((brand: any) => `<span data-brand-item>${esc(brand.brandName)}</span>`).join("")}</div></div>`).join("") || `<div class="app-brand-category" data-brand-category><button class="app-list-card app-category-row" type="button" data-brand-category-toggle><b>👗</b><span>Moda<small>${Math.max(total, 1)} marka</small></span><i>›</i></button></div>` }; };
const feature = (
  label: string,
  title: string,
  copy: string,
