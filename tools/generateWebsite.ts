...
import { appStoreDownloadUrl, googlePlayDownloadUrl } from "../src/constants/appLinks";
...
const navItems = [
  ["/", "Ana Sayfa", "home"],
  ["/explore/", "Keşfet", "explore"],
  ["/outlets/", "Outletler", "outlets"],
  ["/tax-free/", "Tax Free", "savings"],
  ["/trip-planner/", "Seyahat Planı", "trip"],
  ["/flight-deals/", "Uçuş Uyarıları", "trip"],
] as const;
const nav = navItems.map(([href, label, section]) => `<a href="${href}" data-nav-section="${section}">${label}</a>`).join("");
const siteScript = `<script src="/assets/site-interactions.js" defer></script>`;
const quickMenu = `<div class="app-menu-overlay" data-menu-overlay hidden><div class="app-menu-backdrop" data-menu-close></div><section class="app-menu-card" role="dialog" aria-modal="true" aria-labelledby="quick-menu-title"><button class="app-menu-close" type="button" data-menu-close aria-label="Menüyü kapat">×</button><h2 id="quick-menu-title">Hızlı Menü</h2><nav><a href="/outlets/"><span>Outletlere göz at</span><b>→</b></a><a href="/tax-free/"><span>Tax Free Merkezi</span><b>→</b></a><a href="/offline-guide/"><span>Çevrimdışı erişim</span><b>→</b></a><a href="/app/"><span>Uygulama durumu</span><b>→</b></a><a href="/contact/"><span>İletişim</span><b>→</b></a></nav></section></div>`;
const homeQuickMenu = `<div class="app-menu-overlay" data-menu-overlay hidden><div class="app-menu-backdrop" data-menu-close></div><section class="app-menu-card" role="dialog" aria-modal="true" aria-labelledby="quick-menu-title"><button class="app-menu-close" type="button" data-menu-close aria-label="Menüyü kapat">×</button><h2 id="quick-menu-title">Hızlı Menü</h2><nav><a href="/outlets/"><i>🏬</i><span>Outletlere göz at</span><b>→</b></a><a href="/tax-free/"><i>💰</i><span>Tax Free Merkezi</span><b>→</b></a><a href="/offline-guide/"><i>📥</i><span>Çevrimdışı erişim</span><b>→</b></a><a href="/app/"><i>⭐</i><span>Uygulamayı değerlendir</span><b>→</b></a><a href="mailto:info@myoutletguide.com?subject=My%20Outlet%20Guide%20Paylaş"><i>📤</i><span>Uygulamayı paylaş</span><b>→</b></a></nav></section></div>`;
const brandMark = `<img class="brand-mark brand-icon" src="${asset("icon.png")}" alt="" aria-hidden="true">`;
const searchOverlay = `<div class="app-search-overlay" data-search-overlay hidden><div class="app-search-backdrop" data-search-close></div><section class="app-search-card" role="dialog" aria-modal="true" aria-labelledby="search-overlay-title"><button class="app-search-close" type="button" data-search-close aria-label="Aramayı kapat">×</button><p>MY OUTLET GUIDE</p><h2 id="search-overlay-title">Ara</h2><label class="app-search-input"><b>⌕</b><input type="search" placeholder="Şehir, outlet veya marka ara" data-search-input></label><div class="app-search-results" data-search-results></div></section></div>`;
const storeLinks = `<div class="store-links" aria-label="Uygulamayı indir"><a class="store-link" data-store="ios" href="${appStoreDownloadUrl}">App Store’dan indir</a><a class="store-link" data-store="android" href="${googlePlayDownloadUrl}">Google Play’den indir</a></div>`;
const footer = `<footer><div class="footer-grid"><div class="footer-brand">${brandMark}<strong>MY OUTLET GUIDE</strong><p>Outlet keşfi, seyahat planlama ve alışveriş hazırlığı için uygulama odaklı rehber.</p></div><div><p class="footer-label">KEŞFET</p><a href="/">Ana Sayfa</a><a href="/explore/">Keşfet</a><a href="/outlets/">Outletler</a><a href="/tax-free/">Tax Free</a></div><div><p class="footer-label">PLANLA</p><a href="/trip-planner/">Seyahat Planı</a><a href="/flight-deals/">Uçuş Uyarıları</a><a href="/privacy/">Gizlilik</a><a href="/terms/">Şartlar</a></div><div><p class="footer-label">DESTEK</p><a href="/account-deletion/">Hesap Silme</a><a href="/contact/">İletişim</a><a href="mailto:info@myoutletguide.com">info@myoutletguide.com</a><a href="https://instagram.com/myoutletguide">@myoutletguide</a></div></div></footer>`;
function page(
  title: string,
  eyebrow: string,
  h1: string,
  ...
  return `<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} | My Outlet Guide</title><meta name="description" content="${esc(intro)}"><link rel="stylesheet" href="/assets/styles.css"></head><body class="app-screen"><header class="app-topbar"><nav class="app-topbar-nav"><a class="brand app-logo-lockup" href="/">${brandMark}<span>MY OUTLET GUIDE</span></a><div class="links">${nav}</div><a class="nav-app" href="/app/" aria-label="Uygulama durumu">⇩ <span>Uygulama</span></a></nav><section class="hero app-hero-card"><p class="pill app-pill">${eyebrow}</p><h1>${h1}</h1><p>${intro}</p></section></header><main>${body}</main>${footer}${quickMenu}${searchOverlay}${siteScript}</body></html>`;
}
const cta = `<section class="app-cta"><div><p class="eyebrow">MY OUTLET GUIDE UYGULAMASI</p><h2>Planın, rehberin ve favorilerin yanında.</h2><p>Uygulamada favorilerini sakla, seyahatini düzenle ve outlet notlarına eriş. iOS ve Android’de mevcut.</p></div>${storeLinks}</section>`;
...
const prose: Record<string, [string, string, string, string]> = {
  ...
  app: [
    ...
    `<section class="tool-hero"><div><p class="eyebrow">MOBİL DENEYİM</p><h2>Keşfet, planla, yanında taşı.</h2><p>Outlet keşfi, seyahat planlama ve tasarruf araçları uygulama odaklı olarak geliştirilir.</p></div><span>✦</span></section>${cards([card("iOS", "App Store’da şimdi mevcut.", appStoreDownloadUrl, "●"), card("Android", "Google Play’de şimdi mevcut.", googlePlayDownloadUrl, "●")], "status-grid")}<section class="section-heading compact"><p class="eyebrow">UYGULAMA ÖZELLİKLERİ</p><h2>Yanında taşıyabileceğin araçlar</h2></section>${cards([card("Outlet keşfi", "Outlet kartları ve konum notları.", "/explore/", "⌕"), card("Seyahat planı", "Rota ve ziyaret hatırlatmaları.", "/trip-planner/", "◫"), card("Tasarruf araçları", "Tax Free ve maliyet notları.", "/savings/", "₺"), card("Çevrimdışı rehber", "Temel rehber içeriği.", "/offline-guide/", "✓"), card("Uçuş uyarı tercihleri", "Desteklenen rotalar için hazırlık.", "/flight-deals/", "✈")], "feature-grid")}<section class="notice"><h3>Destek</h3><p>info@myoutletguide.com</p></section>`,
  ],
};
...
const searchItems = [
  ...(outlets as any[]).map((outlet) => {
    const city = (cities as any[]).find((item) => item.cityId === outlet.cityId);
    const country = (countries as any[]).find((item) => item.countryId === outlet.countryId);
    const slug = outlet.slug || outlet.outletId;
    return { type: "outlet", title: outlet.name, subtitle: `${trCity(city?.cityName || outlet.cityId)}, ${trCountry(country?.countryName || outlet.countryId)}`, href: `/outlets/${slug}/`, keywords: [outlet.name, city?.cityName, trCity(city?.cityName || ""), country?.countryName, trCountry(country?.countryName || "")].filter(Boolean) };
  }),
  ...(cities as any[]).map((city) => {
    const country = (countries as any[]).find((item) => item.countryId === city.countryId);
    return { type: "city", title: trCity(city.cityName), subtitle: `${trCountry(country?.countryName || city.countryId)} · ${outlets.filter((outlet: any) => outlet.cityId === city.cityId).length} outlet`, href: `/cities/${city.cityId}/`, keywords: [city.cityName, trCity(city.cityName), country?.countryName, trCountry(country?.countryName || "")].filter(Boolean) };
  }),
  ...(countries as any[]).map((country) => ({ type: "country", title: trCountry(country.countryName), subtitle: `${outlets.filter((outlet: any) => outlet.countryId === country.countryId).length} outlet · ${country.currency}`, href: `/countries/${country.countryId}/`, keywords: [country.countryName, trCountry(country.countryName), country.currency].filter(Boolean) })),
  ...(brands as any[]).filter((brand) => brand.brandStatus === "active").map((brand) => ({ type: "brand", title: brand.brandName, subtitle: "Marka", href: "/brands/", keywords: [brand.brandName, ...(Array.isArray(brand.aliases) ? brand.aliases : [])].filter(Boolean) })),
];
mkdirSync(`${web}/assets`, { recursive: true });
writeFileSync(`${web}/assets/search-index.json`, `${JSON.stringify(searchItems, null, 2)}\n`);
...
const trRestaurantCategory = formatTurkishRestaurantCategory;
...
const brandCategoryRows = (outletId: string) => { const related = outletBrands.filter((x: any) => x.outletId === outletId && x.relationStatus === "active"); const grouped = new Map<string, any[]>(); for (const rel of related) { const brand = (brands as any[]).find((b) => b.brandId === rel.brandId); if (!brand) continue; const id = brand?.luxuryLevel === "luxury" ? "luxury" : brand?.categoryId || "fashion"; grouped.set(id, [...(grouped.get(id) || []), brand]); } const total = related.length; const rows = Object.entries(categoryLabels).map(([id, [icon, label]]) => ({ id, icon, label, items: (grouped.get(id) || []).sort((a,b) => a.brandName.localeCompare(b.brandName, "tr")) })).filter((row) => row.items.length > 0); return { total, html: rows.map((row, index) => `<div class="app-brand-category" data-brand-category><button class="app-list-card app-category-row" type="button" data-brand-category-toggle><b>${row.icon}</b><span>${row.label}<small>${row.items.length} marka</small></span><i>›</i></button><div class="app-brand-chip-list" hidden>${row.items.map((brand: any) => `<span data-brand-item>${esc(brand.brandName)}</span>`).join("")}</div></div>`).join("") || `<div class="app-brand-category" data-brand-category><button class="app-list-card app-category-row" type="button" data-brand-category-toggle><b>👗</b><span>Moda<small>${Math.max(total, 1)} marka</small></span><i>›</i></button></div>` }; };
const feature = (
  label: string,
  title: string,
  copy: string,
  ...
  `<a class="app-feature-card" href="${href}" style="background-image:linear-gradient(0deg,rgba(8,29,56,.88),rgba(8,29,56,.15)),url('${asset(image)}')"><span>${label}</span><strong>${title}</strong><em>${copy}</em><b>${ctaLabel}</b></a>`;

const homePage = (body: string) => `<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Outlet keşfi | My Outlet Guide</title><meta name="description" content="Premium outletleri keşfet, daha çok tasarruf et ve sonraki alışveriş seyahatini planla."><link rel="stylesheet" href="/assets/styles.css"></head><body class="home-page app-screen"><header class="home-header app-topbar" style="background-image:linear-gradient(90deg,rgba(4,19,39,.96) 0%,rgba(8,29,56,.82) 47%,rgba(8,29,56,.45) 100%),url('${asset("home/home-hero-premium.png")}')"><nav class="home-nav app-topbar-nav"><button class="home-menu-button" type="button" data-menu-open aria-label="Menüyü aç">☰</button><a class="brand app-logo-lockup" href="/">${brandMark}<span>MY OUTLET GUIDE</span></a><div class="links home-links"><a class="is-active" href="/" aria-current="page">Ana Sayfa</a><a href="/explore/">Keşfet</a><a href="/outlets/">Outletler</a><a href="/tax-free/">Tax Free</a><a href="/trip-planner/">Seyahat Planı</a><a href="/flight-deals/">Uçuş Uyarıları</a><a href="/app/">Uygulama</a></div><div class="home-utility-actions"><a class="home-icon-button" href="/app/" aria-label="Bildirimler">🔔</a><a class="nav-app" href="/app/" aria-label="Dil ve uygulama durumu">🇹🇷 <span>TR</span></a></div></nav><section class="hero home-main-hero app-hero-card app-image-card"><div class="home-hero-brand">${brandMark}<p class="pill app-pill">PREMIUM ALIŞVERİŞ VE SEYAHAT ASİSTANI</p></div><h1>My Outlet Guide’a hoş geldin</h1><p>Premium outletleri keşfet, daha çok tasarruf et ve sonraki alışveriş seyahatini planla.</p></section><section class="home-inline-search" aria-label="Ana sayfa araması"><label class="home-search app-search-pill"><b>⌕</b><input type="search" placeholder="Şehir, outlet ve marka ara" data-search-input data-inline-search data-home-inline-search><button class="home-search-clear" type="button" data-search-clear aria-label="Aramayı temizle">×</button></label><div class="app-search-results home-inline-results" data-search-results data-home-search-results hidden></div></section><div class="home-hero-actions"><a class="home-primary-cta" href="/explore/">Outletleri keşfet <b>→</b></a><a class="home-status-chip" data-store="ios" href="${appStoreDownloadUrl}">App Store’dan indir</a><a class="home-status-chip" data-store="android" href="${googlePlayDownloadUrl}">Google Play’den indir</a></div></header><main class="home-main"><div class="home-content home-wide-container app-screen app-page">${body}</div>${appTabs}</main>${footer}${homeQuickMenu}${searchOverlay}${siteScript}</body></html>`;


const exploreCityImages: Record<string, string> = { paris: "Paris.webp", milan: "Milano.webp", london: "London.webp", munich: "Munich.webp" };
const exploreRows = {
...
const exploreDataAttr = esc(JSON.stringify(exploreRows));
const explorePopularCities = ["paris", "milan", "london", "munich"].map((id) => { const row = exploreRows.cities.find((city) => city.id === id); return row ? { ...row, image: exploreCityImages[id] ? asset(`city-images/${exploreCityImages[id]}`) : "" } : row; }).filter(Boolean);
const exploreListCard = (item: any) => `<a class="explore-list-card" href="${item.href}" data-explore-card><span class="explore-avatar">${item.image ? `<img src="${item.image}" alt="">` : esc(item.flag || "⌖")}</span><span>${esc(item.title)}<small>${esc(item.subtitle)}</small></span><i>→</i></a>`;
...
const recommendedOutletPriority = [
  "la-vallee-village",
  "bicester-village",
  "serravalle-designer-outlet",
  "designer-outlet-roermond",
];
const recommendedOutlets = recommendedOutletPriority
  .map((slug) =>
    (outlets as any[]).find((outlet) => (outlet.slug || outlet.outletId) === slug),
  )
  .filter(Boolean);
const homeOutletCard = (outlet: any) =>
  outletCard(outlet)
    .replace("PREMİUM OUTLET", "PREMIUM OUTLET")
    .replace("Amsterdam · Hollanda", "Roermond · Hollanda");

write(
  "",
  homePage(`<section class="app-section home-feature-section"><h2>Öne çıkanlar</h2><p>Outlet keşfi, seyahat planı, tasarruf ve çevrimdışı erişim için temel araçlar.</p><div class="app-feature-grid home-rail" data-carousel="featured">${feature("KEŞFET", "Outletleri keşfet", "Premium markalar ve destinasyonlar.", "/explore/", "explore/explore-hero-premium.png", "Outletleri keşfet →")}${feature("SEYAHAT", "Outlet seyahatini planla", "Rotanı kartlarla düzenle.", "/trip-planner/", "heroes/hero-trips.png", "Planla →")}${feature("TASARRUF", "Tasarruf araçları", "Tax Free ve fiyat notların.", "/savings/", "heroes/hero-savings.png", "Araçları aç →")}${feature("REHBER", "Çevrimdışı rehber", "Temel bilgileri yanında tut.", "/offline-guide/", "heroes/hero-offline.png", "Rehberi gör →")}</div><div class="home-rail-dots" aria-label="Öne çıkanlar sayfaları"><span data-carousel-dot aria-current="true"></span><span data-carousel-dot></span><span data-carousel-dot></span><span data-carousel-dot></span></div></section><section class="app-section home-recommended-section"><h2>Önerilen outletler</h2><p>Bir sonraki alışveriş günün için yüksek ilgi gören premium duraklar.</p><div class="app-outlet-grid home-rail" data-carousel="recommended">${recommendedOutlets.map(homeOutletCard).join("")}</div><div class="home-rail-dots" aria-label="Önerilen outletler sayfaları"><span data-carousel-dot aria-current="true"></span><span data-carousel-dot></span><span data-carousel-dot></span><span data-carousel-dot></span></div></section><section class="app-section home-activity-section"><h2>Aktiviteniz</h2><p>Kayıtlı alışveriş planlarınıza hızlı erişim.</p><div class="app-card activity-card"><div><i>🧳</i><span>Alışveriş Seyahati</span><b>Henüz seyahat yok</b><small>Hatırlatıcılar için bir seyahat oluşturun.</small><a href="/trip-planner/">Seyahat oluştur</a></div><div><i>♡</i><span>Favoriler</span><b>Henüz favori outlet yok</b><small>Takip etmek istediğiniz outletleri kaydedin.</small><a href="/explore/">Favorilere başla</a></div></div></section><section class="app-section home-tools-section"><h2>Alışveriş araçları</h2><p>Alışverişten önce tasarrufunu planlayın.</p><div class="app-quick-grid home-tool-grid">${[["Tax Free Hesaplayıcı", "₺", "İadeni tahmin et ve minimum harcamayı kontrol et.", "/tax-free/"], ["Döviz Çevirici", "⇄", "Alışveriş para birimini ayarla.", "/savings/"], ["Uçuş Alarmı", "✈", "Uçuş fiyat izleme durumunu gör.", "/flight-deals/"], ["Çevrimdışı", "↓", "İnternetsiz çalışan rehber özelliklerini gör.", "/offline-guide/"]].map(([title, icon, copy, href]) => `<a class="app-quick-card home-tool-card" href="${href}"><b class="home-tool-icon">${icon}</b><span>${title}</span><small>${copy}</small></a>`).join("")}</div></section><section class="app-section"><h2>Popüler şehirler</h2><p>Şehir rehberleriyle outlet rotanı şekillendir.</p><div class="app-city-list home-city-grid home-rail" data-carousel="cities">${homeCities.map(([city, country, image, cityId], index) => { const counts = [2, 2, 3, 1]; return `<a class="app-list-card home-city-card" href="/cities/${cityId}/" style="background-image:linear-gradient(90deg,rgba(8,29,56,.86),rgba(8,29,56,.25)),url('${asset(`city-images/${image}`)}')"><span>${city}<small>${country} · ${cityOutletCount(counts[index])}</small></span><i>→</i></a>`; }).join("")}</div><div class="home-rail-dots" aria-label="Popüler şehirler sayfaları"><span data-carousel-dot aria-current="true"></span><span data-carousel-dot></span><span data-carousel-dot></span><span data-carousel-dot></span></div></section><section class="app-cta home-final-cta"><div><p class="eyebrow">MY OUTLET GUIDE UYGULAMASI</p><h2>Planın, rehberin ve favorilerin yanında.</h2><p>Uygulamada favorilerini sakla, seyahatini düzenle ve outlet notlarına eriş.</p></div>${storeLinks}</section>`),
);

write(
  "explore",
  `<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Keşfet | My Outlet Guide</title><meta name="description" content="Ülkeleri, şehirleri, outletleri ve markaları tek premium keşif merkezinde ara."><link rel="stylesheet" href="/assets/styles.css"></head><body class="app-screen"><main><div class="app-screen app-shell app-page"><div class="explore-app" data-explore-page data-explore-data="${exploreDataAttr}"><section class="app-image-hero explore-hero" style="background-image:linear-gradient(0deg,rgba(8,29,56,.88),rgba(8,29,56,.12)),url('${asset("explore/explore-hero-premium.png")}')"><p>KEŞFET</p><h2>Aklındakini bul</h2><span>Ülkeleri, şehirleri, outletleri ve markaları tek premium keşif merkezinde ara.</span></section><label class="app-search-input explore-search-input"><b>⌕</b><input type="search" placeholder="Ülke, şehir, outlet, marka ara..." data-search-input data-inline-search data-explore-main-search><button class="explore-search-clear" type="button" data-search-clear aria-label="Aramayı temizle" hidden>×</button></label><section class="app-section app-search-section explore-results-panel" data-search-section hidden><h2>Arama Sonuçları</h2><p data-search-count></p><div class="app-search-results" data-search-results></div></section><div class="app-chip-row explore-mode-chips" data-explore-modes><button class="app-chip" type="button" data-explore-mode="country">🌍 Ülkeler</button><button class="app-chip" type="button" data-explore-mode="city">📍 Şehirler</button><button class="app-chip" type="button" data-explore-mode="outlet">🛍️ Outletler</button></div><div data-default-discovery><section class="app-section"><h2>Popüler aramalar</h2><div class="app-chip-row explore-popular-chips">${["Paris", "Burberry", "Fransa", "İtalya", "Nike"].map((x) => `<button type="button" class="app-chip" data-search-chip="${x}">${x}</button>`).join("")}</div></section><section class="app-section explore-discovery"><h2>Nasıl keşfetmek istersin?</h2><div class="explore-discovery-grid"><button class="explore-discovery-card is-wide" type="button" data-explore-mode="outlet"><b>🛍️</b><span>Outlet ara<small>Outlet, şehir, ülke veya marka adına göre ara.</small></span></button><button class="explore-discovery-card" type="button" data-explore-mode="country"><b>🌍</b><span>Ülkeye göre keşfet<small>Outletleri ülkelere göre incele.</small></span></button><button class="explore-discovery-card" type="button" data-explore-mode="city"><b>📍</b><span>Şehre göre keşfet<small>Outlet kapsamı olan şehirleri bul.</small></span></button></div></section><section class="app-section"><h2>Popüler şehirler</h2><p>Premium outlet erişimi olan şehirleri keşfet.</p><div class="explore-list-stack">${explorePopularCities.map((city) => exploreListCard(city)).join("")}</div></section></div><section class="app-section explore-mode-panel" data-explore-mode-panel hidden><h2 data-mode-title></h2><p data-mode-subtitle></p><label class="app-search-input explore-mode-search"><b>⌕</b><input type="search" data-mode-search placeholder="Ülke ara..."></label><div class="app-chip-row explore-country-filters" data-country-filters hidden></div><p class="explore-result-count" data-mode-count></p><div class="explore-list-stack" data-mode-results></div></section></div></div>${appTabs}</main>${footer}${quickMenu}${searchOverlay}${siteScript}</body></html>`,
);
...
write(
  ...
  shell(
    ...
    "OUTLET REHBERİ",
    "Outletleri keşfet",
    "Yer, çalışma saatleri, hizmetler ve seyahat notlarını uygulama kartlarında incele.",
    `<section class="app-dark-card"><small>OUTLET REHBERİ</small><h2>Outletleri keşfet</h2><p>Yer, çalışma saatleri, hizmetler ve seyahat notlarını uygulama kartlarında incele.</p></section><a class="app-search-pill" href="/explore/">⌕ <span>Ülke, şehir, outlet, marka ara...</span></a><section class="app-section app-discovery-list"><h2>Tüm outletler</h2><p>Her outlet detay sayfası aynı mobil uygulama görsel kabuğunu kullanır.</p><div class="app-outlet-grid">${outlets.map(outletCard).join("")}</div></section>`,
  ),
);
...
for (const outlet of outlets as any[]) {
  ...
  const gallery = heroFile
    ? `<img data-gallery-hero src="${asset(`outlet-images/${slug}/${heroFile}`)}" alt="${esc(outlet.name)}">`
    : "";
  const thumbnailStrip = images.length
    ? `<div class="app-gallery-strip" data-gallery-strip>${images.slice(0, 5).map((file, index) => `<button type="button" class="app-gallery-thumb ${index === 0 ? "is-selected" : ""}" data-gallery-thumb data-src="${asset(`outlet-images/${slug}/${file}`)}" aria-label="${esc(outlet.name)} galeri görseli ${index + 1}"><img src="${asset(`outlet-images/${slug}/${file}`)}" alt="${esc(outlet.name)} galeri görseli"></button>`).join("")}</div>`
    : "";
  const airportSummary = outlet.airports?.length
  ...
  const quickFacts = [
    ...
    ["⌖", "ŞEHİR MERKEZİ", `${outlet.cityCenterDistanceKm ?? "—"} km`],
    ["★", "PUAN", "Henüz puan yok"],
  ];
  const route = (from: string) => `<article class="app-route-card app-tool-card"><span>${from}</span><b>${from.startsWith("Hava") ? "Havalimanından taksi/Uber ile" : "Taksi / Uber ile"}</b><div class="app-chip-row"><em>Taksi / Uber</em><em>Yaklaşık 25–50 dk</em><em>Yaklaşık €35–75</em></div><small>Güncel saat ve ücretleri seyahat öncesi kontrol edin.</small></article>`;
  write(
    `outlets/${slug}`,
    outletDetailPage(
      `${outlet.name} outlet rehberi`,
      `${trCity(city?.cityName || outlet.cityId)}, ${trCountry(country?.countryName || outlet.countryId)}`,
      (() => { const loc = `${esc(trCity(city?.cityName || outlet.cityId))}, ${esc(trCountry(country?.countryName || outlet.countryId))}`; const tax = taxFreeRules.find((rule: any) => rule.countryId === outlet.countryId); const brandRows = brandCategoryRows(outlet.outletId); const food = (restaurants as any[]).filter((r) => r.outletId === outlet.outletId && r.status !== "inactive").sort((a,b) => Number(a.displayOrder || 99) - Number(b.displayOrder || 99)); const services = [...new Set((outlet.services || ["Parking", "Tax Free", "Guest Services", "Restaurants and Cafes", "Shuttle / Transport Information", "Wi-Fi", "Gift Cards", "Accessibility", "ATM", "Personal Shopping"]).map((x: string) => trService(x)).filter(Boolean))]; const mapRows = [["🗺️", "Google Maps’te Aç", "Önerilen rota", outlet.googleMapsUrl, "is-primary"], ["", "Apple Maps’te Aç", "iOS harita seçeneği", outlet.appleMapsUrl, ""], ["⌖", "Yandex Maps’te Aç", "Alternatif harita seçeneği", outlet.yandexMapsUrl, ""]]; return `<section class="app-detail-hero app-image-card">${gallery}<div><p>PREMIUM OUTLET</p><h2>${esc(outlet.name)}</h2><span>${loc}</span></div></section>${thumbnailStrip}<span class="app-status">● Aktif</span><div class="app-action-grid"><a class="app-action-card app-cta" href="/app/"><span>♡</span><b>Favori</b></a><a class="app-action-card app-cta" href="/trip-planner/"><span>◫</span><b>Seyahat Oluştur</b></a><a class="app-action-card app-cta" href="${esc(outlet.googleMapsUrl || '/explore/')}"><span>⌖</span><b>Yol Tarifi</b></a></div><div class="app-chip-row app-detail-tabs">${["Özet", "Markalar", "Ulaşım", "Yemek", "Yorumlar"].map((x, i) => `<a href="#${i ? x.toLocaleLowerCase('tr-TR') : 'ozet'}" class="app-tab-chip ${i === 0 ? "selected" : ""}">${x}</a>`).join("")}</div><section id="ozet" class="app-card app-white-card app-info-card app-quick-facts"><h2>Hızlı Bilgiler</h2><div class="app-quick-grid">${quickFacts.map(([icon, label, value]) => `<div class="app-quick-fact"><i>${icon}</i><small>${label}</small><b>${value}</b></div>`).join("")}</div></section><section class="app-card app-white-card app-tax-card"><h2>Tax Free</h2><p>KDV oranı: ${tax?.vatRate ?? "—"}%</p>${tax?.minimumPurchaseAmount ? `<p>Minimum harcama: ${tax.minimumPurchaseAmount} ${tax.currency}</p>` : ""}<small>Tahmini iade garanti edilen iade değildir; gerçek iade mağaza, sağlayıcı, işlem ücretleri, ülke kuralları ve uygunluğa göre daha düşük olabilir.</small></section><section id="markalar" class="app-card app-white-card app-brands-card"><div class="app-section-title"><h2>Markalar</h2><small>${brandRows.total || ""} marka</small></div><label class="app-search-pill app-brand-search">⌕ <input type="search" placeholder="Marka ara..." data-brand-search></label>${brandRows.html}</section><section id="ulaşım" class="app-card app-white-card"><h2>Ulaşım</h2><p>Bu outlet'e ulaşmanın pratik yolları.</p>${route("Şehir merkezinden")}${route("Havaalanından")}<a class="button app-navy-button" href="/outlets/${slug}/transport/" data-transport-link>Ulaşım Rehberini Gör</a></section><section id="haritalar" class="app-card app-white-card app-map-card"><h2>Haritalar</h2><p>Navigasyonu tercih ettiğin harita uygulamasıyla aç.</p>${mapRows.map(([icon,title,sub,url,klass]) => url ? `<a class="app-list-card app-map-row ${klass}" href="${esc(url)}"><b>${icon}</b><span>${title}<small>${sub}</small></span><i>›</i></a>` : `<span class="app-list-card app-map-row"><b>${icon}</b><span>${title}<small>Adres bilgisini resmi kaynaklardan kontrol edin</small></span></span>`).join("")}</section><section class="app-card app-white-card app-official-card"><h2>🌐 Resmi Web Sitesi</h2><p>Güncel çalışma saatleri, etkinlikler, kampanyalar, marka güncellemeleri ve geçici duyurular için her zaman resmi web sitesini kontrol edin.</p>${outlet.websiteUrl ? `<a class="button app-navy-button" href="${esc(outlet.websiteUrl)}">Resmi Web Sitesini Ziyaret Et</a>` : ""}</section><section id="yemek" class="app-card app-white-card"><div class="app-section-title"><h2>Restoranlar & Kafeler</h2><small>${food.length} yer</small></div>${food.map((x: any, index: number) => `<div class="app-list-card app-food-row" ${index > 2 ? "data-extra-restaurant hidden" : ""}><b>☕</b><span>${esc(x.restaurantName)}<small>${esc(trRestaurantCategory(x.category || 'Restoran'))}${x.priceLevel ? ` · ${esc(x.priceLevel)}` : ''}</small></span></div>`).join("")}${food.length > 3 ? `<button class="button app-navy-button" type="button" data-restaurants-toggle>Tüm restoranlar</button>` : ""}</section><section class="app-card app-white-card"><h2>Hizmetler</h2><div class="app-chip-row app-service-list">${services.map((x) => `<span class="app-service-chip">✓ ${esc(x)}</span>`).join("")}</div></section><section id="yorumlar" class="app-card app-white-card reviews-card"><div class="app-section-title"><h2>Yorumlar</h2><a href="#yorumlar">Yorum yaz</a></div><p>Outlet alışveriş deneyimini paylaş.</p><div class="app-dark-card"><small>GENEL PUAN</small><b>⭐ 0.0 <em>(0 yorum)</em></b></div><div class="app-quick-grid">${["Ulaşım", "Markalar", "Restoranlar", "Hizmetler"].map((x) => `<div class="app-quick-card"><span>${x}</span><b>—</b></div>`).join("")}</div><div class="app-chip-row"><span class="app-tab-chip selected">En faydalı</span><span class="app-tab-chip">En yeni</span></div><p>Henüz yorum yok.</p></section>`; })(),
    ),
  );
  const transportBody = `<section class="app-transport-page"><h1>Ulaşım</h1><p class="transport-gold">Bu outlet'e ulaşmanın en pratik yolları.</p><section class="app-card app-white-card app-recommended-route"><p class="eyebrow">Önerilen Rota</p><h2>Taksi / Uber ile</h2><div class="app-chip-row"><em>Taksi / Uber</em><em>Yaklaşık 25–50 dk</em><em>Yaklaşık €35–75</em></div><div class="app-list-card"><span>Başlangıç<small>Şehir merkezi veya havalimanı</small></span></div><div class="app-list-card"><span>Varış<small>${esc(outlet.name)}</small></span></div><small>Güncel saat ve ücretleri seyahat öncesi kontrol edin.</small></section><section class="app-section"><h2>Havaalanından ulaşım</h2>${["Havalimanından toplu ulaşım ile", "Havalimanından taksi/Uber ile"].map((x) => `<article class="app-route-card app-tool-card"><b>${x}</b><div class="app-chip-row"><em>Rota</em><em>Yaklaşık süre</em><em>Yaklaşık ücret</em></div><small>Resmi sağlayıcı ve harita bilgilerini seyahat öncesi kontrol edin.</small></article>`).join("")}</section><section class="app-section"><h2>Şehir merkezinden ulaşım</h2>${["Şehir merkezinden trenle / toplu ulaşım", "Taksi / Uber ile"].map((x) => `<article class="app-route-card app-tool-card"><b>${x}</b><div class="app-chip-row"><em>Rota</em><em>Yaklaşık süre</em><em>Yaklaşık ücret</em></div><small>Canlı fiyat iddiası olmadan statik rota özeti.</small></article>`).join("")}</section><section class="app-card app-white-card"><h2>Adım adım rehber</h2><ol><li>Başlangıç: şehir merkezi veya havalimanı noktasını seç.</li><li>Varış: outlet adını veya konumunu ayarla.</li><li>Yaklaşık ücreti yolculuk başlamadan önce uygulamada kontrol et.</li><li>Trafik ve dönüş saatini alışverişten önce tekrar kontrol et.</li></ol></section><section class="app-card app-white-card app-map-card"><h2>Harita / navigasyon</h2><a class="app-list-card app-map-row is-primary" href="${esc(outlet.googleMapsUrl || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(outlet.name)}`)}"><b>🗺️</b><span>Google Maps<small>Önerilen rota</small></span><i>›</i></a><a class="app-list-card app-map-row" href="${esc(outlet.appleMapsUrl || `https://maps.apple.com/?q=${encodeURIComponent(outlet.name)}`)}"><b></b><span>Apple Maps<small>Haritada aç</small></span><i>›</i></a></section></section>`;
  write(`outlets/${slug}/transport`, `<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Ulaşım | ${esc(outlet.name)}</title><meta name="description" content="${esc(outlet.name)} ulaşım rehberi"><link rel="stylesheet" href="/assets/styles.css"></head><body class="app-screen outlet-detail-page"><header class="mobile-detail-topbar"><a href="/outlets/${slug}/" data-history-back>‹ Geri</a><strong>Ulaşım</strong><span></span></header><main><div class="app-screen app-shell app-page outlet-detail-shell">${transportBody}</div>${appTabs}</main>${footer}${siteScript}</body></html>`);
}
