...
function page(
  ...
  return `<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} | My Outlet Guide</title><meta name="description" content="${esc(intro)}"><link rel="stylesheet" href="/assets/styles.css"></head><body class="app-screen"><header class="app-topbar"><nav class="app-topbar-nav"><a class="brand app-logo-lockup" href="/"><span class="brand-mark" aria-hidden="true"><svg viewBox="0 0 32 32"><path d="M8 12h16l-1 13H9L8 12Z"/><path d="M12 12a4 4 0 0 1 8 0"/><path d="m16 15 1.8 3.5 3.8.5-2.8 2.7.7 3.8-3.5-1.8-3.5 1.8.7-3.8-2.8-2.7 3.8-.5L16 15Z"/></svg></span><span>MY OUTLET GUIDE</span></a><div class="links">${nav}</div><a class="nav-app" href="/app/" aria-label="Uygulama durumu">⇩ <span>Uygulama</span></a></nav><section class="hero app-hero-card"><p class="pill app-pill">${eyebrow}</p><h1>${h1}</h1><p>${intro}</p></section></header><main>${body}</main>${footer}${quickMenu}${searchOverlay}${siteScript}</body></html>`;
}
...
write(
  ...
  shell(
    ...
    `<section class="app-image-hero explore-hero" style="background-image:linear-gradient(0deg,rgba(8,29,56,.88),rgba(8,29,56,.12)),url('${asset("explore/explore-hero-premium.png")}')"><p>KEŞFET</p><h2>Aklındakini bul</h2><span>Ülkeleri, şehirleri, outletleri ve markaları tek premium keşif merkezinde ara.</span></section><label class="app-search-input explore-search-input"><b>⌕</b><input type="search" placeholder="Ülke, şehir, outlet, marka ara..." data-search-input data-inline-search></label><div class="app-chip-row" data-type-filters><button class="app-chip" type="button" data-search-type="country">🌍 Ülkeler</button><button class="app-chip" type="button" data-search-type="city">📍 Şehirler</button><button class="app-chip" type="button" data-search-type="outlet">🛍️ Outletler</button><button class="app-chip" type="button" data-search-type="brand">🏷️ Markalar</button></div><section class="app-section" data-default-discovery><h2>Popüler aramalar</h2><div class="app-chip-row">${["Paris", "Burberry", "Fransa", "İtalya", "Nike"].map((x) => `<button type="button" class="app-chip" data-search-chip="${x}">${x}</button>`).join("")}</div></section><section class="app-section app-search-section" data-search-section hidden><h2>Arama sonuçları</h2><div class="app-search-results" data-search-results></div></section><section class="app-section"><h2>Nasıl keşfetmek istersin?</h2><div class="app-quick-grid">${[
      "Outlet ara|⌕|/outlets/",
