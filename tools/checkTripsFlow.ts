...
assert(createTrip.includes("useEffect") && createTrip.includes("requireAuth({ isLoggedIn, navigation"), "CreateTripScreen entry must auth-gate guests");
...
assert(createTrip.includes("returnFlightNumber") && createTrip.includes("returnDepartureAirport") && createTrip.includes("returnDepartureDate") && createTrip.includes("returnDepartureTime") && !/outbound|airline|arrivalAirport/i.test(createTrip), "CreateTripScreen must support only minimal optional return-flight fields");
assert(createTrip.includes("notes") && createTrip.includes("routeInfoBody"), "CreateTripScreen must support notes and informational route context");
...
assert(reminderService.includes("generateTripReminderPlan") && reminderService.includes("segmentStart") && reminderService.includes("taxFreeOneDayBeforeEnd") && reminderService.includes("returnFlight"), "reminder plan generation must be centralized and cover segment, Tax Free, and conditional return-flight reminders");
...
assert(combinedTripSource.includes("taxFreeReminder") && combinedTripSource.includes("returnFlightReminder"), "reminder plan must include Tax Free and return-flight reminders when relevant");
