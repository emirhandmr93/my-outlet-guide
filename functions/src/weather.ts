...
async function fetchLocation(location: ReturnType<typeof validateLocations>[number], baseUrl: string, apiKey: string) {
...
export const getTripWeather = onCall({ region: "us-central1", memory: "256MiB", timeoutSeconds: 30 }, async (request) => {
  ...
  const baseUrl = process.env.OPEN_METEO_BASE_URL || DEFAULT_BASE_URL;
  if (!apiKey || !baseUrl) return { provider: "Open-Meteo", updatedAt: Timestamp.now().toDate().toISOString(), status: "provider_not_configured" as WeatherStatus, locations: [] };
  try {
    const results = await Promise.all(locations.map((location) => fetchLocation(location, baseUrl, apiKey)));
